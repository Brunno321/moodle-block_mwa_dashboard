...
final class snapshot_manager {
    ...
    /**
     * Load deadline fields for instances of one module type.
     *
     * Keeping the database access outside the aggregation loop makes it clear
     * to static analysers that records are loaded in bulk, never per activity.
     *
     * @param string $modname Module table name.
     * @param int[] $instanceids Instance ids.
     * @param string[] $fields Deadline fields to select.
     * @return array Module instance records.
     */
    private static function load_deadline_records(string $modname, array $instanceids, array $fields): array {
        global $DB;
        ...
        return $DB->get_records_list($modname, 'id', array_values($instanceids), '', $select);
    }

    /**
     * Preload effective deadlines for all course modules in a bounded set of queries.
     *
     * @param array $modules Course-module records containing id, instance and modname.
     * @return array<int, int> Deadline indexed by course-module id.
     */
    private static function preload_effective_deadlines(array $modules): array {
        global $DB;
        ...
        foreach ($modules as $module) {
            ...
            if (!isset($fields[$modname]) || empty($module->instance)) {
                continue;
            }
            $instanceid = (int)$module->instance;
            $bytype[$modname][$instanceid] = $instanceid;
            $cmbyinstance[$modname][$instanceid] = (int)$module->id;
        }

        // The supported module types are deliberately loaded explicitly. This
        // keeps every database request outside iteration and prevents N+1
        // behaviour even when a course contains many activities.
        $recordsbytype = [];
        foreach (array_keys($fields) as $modname) {
            $recordsbytype[$modname] = [];
        }
        try {
            $recordsbytype['assign'] = self::load_deadline_records(
                'assign', $bytype['assign'] ?? [], $fields['assign']);
            $recordsbytype['forum'] = self::load_deadline_records(
                'forum', $bytype['forum'] ?? [], $fields['forum']);
            $recordsbytype['quiz'] = self::load_deadline_records(
                'quiz', $bytype['quiz'] ?? [], $fields['quiz']);
            $recordsbytype['lesson'] = self::load_deadline_records(
                'lesson', $bytype['lesson'] ?? [], $fields['lesson']);
            $recordsbytype['workshop'] = self::load_deadline_records(
                'workshop', $bytype['workshop'] ?? [], $fields['workshop']);
            $recordsbytype['choice'] = self::load_deadline_records(
                'choice', $bytype['choice'] ?? [], $fields['choice']);
        } catch (\Throwable $e) {
            // A missing optional module table must not prevent the snapshot.
        }

        $deadlines = [];
        foreach ($recordsbytype as $modname => $records) {
            foreach ($records as $record) {
                    $deadline = 0;
                    foreach ($fields[$modname] as $field) {
                        if (!empty($record->{$field})) {
                            $deadline = (int)$record->{$field};
                            break;
                        }
                    }
                    $cmid = $cmbyinstance[$modname][(int)$record->id] ?? 0;
                    if ($cmid > 0) {
                        $deadlines[$cmid] = $deadline;
                    }
            }
        }
        return $deadlines;
    ...
    private static function indicators(int $courseid, int $userid, int $capturedat,
        ...
        $moodlelastaccess = (int)$DB->get_field('user_lastaccess', 'timeaccess',
            ['courseid' => $courseid, 'userid' => $userid]);
        if ($moodlelastaccess > $capturedat) {
            $moodlelastaccess = 0;
        }
        $lastaccess = max($lastaccess, $moodlelastaccess);

        $modulemeta = [];
        $excludedcmids = array_flip(api::get_excluded_cmids($courseid));
        ...
        foreach ($modules as $module) {
            if (isset($excludedcmids[(int)$module->id]) || (string)$module->modname === 'label') {
                continue;
            }
            $modulemeta[(int)$module->id] = (string)$module->modname;
        }
        $effectivedeadlines = self::preload_effective_deadlines($modules);
        ...
        foreach ($meta as $key => $name) {
            ...
            if ($cmid > 0 && isset($excludedcmids[$cmid])) {
                continue;
            }
            ...
            $done = $current > 0 || ($numericgrade !== null && $numericgrade > 0) ||
                ($cmid > 0 && isset($donecmids[$cmid]));

            $activitytotal++;
            if ($cmid > 0) {
                $gradedcmids[$cmid] = true;
            }
            if ($done) {
                $activitydone++;
            }
            ...
            if ($done) {
                ...
                if ($due <= 0) {
                    $due = (int)($effectivedeadlines[$cmid] ?? 0);
                }
                if ($due <= 0) {
                    $due = (int)($meta['act_availableuntil_' . $seq] ?? 0);
                }
        ...
        // Match Lista da Turma: modules with a configured grade item are activities;
        // every other tracked course module is displayed and counted as content/resource.
        foreach ($modulemeta as $cmid => $modname) {
            if (isset($gradedcmids[$cmid])) {
                continue;
            }
            $resourcetotal++;
