...
class external extends \external_api {
    ...
    public static function get_ai_recommendation(int $courseid, string $student_name, string $prompt): array {
        ...
        if (strpos($instruction, '[Texto completo dos alunos') !== false ||
            ...
            $auditcategories[] = 'forum_posts_anonymised';
        }
        if (strpos($instruction, 'QUESTÕES DO QUIZ') !== false) {
            $auditcategories[] = 'question_text_only';
            if (strpos($instruction, '[CORRETA]') !== false &&
                    has_capability('block/mwa_dashboard:viewanswers', $context)) {
                $auditcategories[] = 'question_answer_key';
            }
        }
        try {
            ...
            case 'quiz':
                ...
                if ($rec) {
                    ...
                    if ($questions) {
                        ...
                        // Resolved once per request (Moodle caches capability checks internally).
                        $includeanswerkey = has_capability('block/mwa_dashboard:viewanswers', $context);
                            ...
                            try {
                                ...
                                if ($answers) {
                                    foreach ($answers as $a) {
                                        ...
                                        if ($alt === '') continue;
                                        $mark = ($includeanswerkey && (float)($a->fraction ?? 0) > 0) ? ' [CORRETA]' : '';
                                        $q_block .= "\n   - $alt$mark";
                                    }
                                }
                            } catch (\Throwable $e) {}
