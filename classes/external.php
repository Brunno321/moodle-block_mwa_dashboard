...
class external extends \external_api {
    ...
    /**
     * Remove enrolled student identifiers from chat text.
     *
     * Chat uses class aggregates only. Unlike individual AI recommendations,
     * student aliases are not useful here because they would still permit
     * external profiling of a persistent individual.
     *
     * @param int $courseid Course identifier.
     * @param string $text Message text.
     * @return string Text without enrolled student names or email addresses.
     */
    private static function redact_students_from_chat(int $courseid, string $text): string {
        foreach (self::get_ai_enrolled_users($courseid) as $student) {
            $identifiers = [
                trim($student->firstname . ' ' . $student->lastname),
                ...
                self::get_ai_alias($courseid, (int)$student->id),
            ];
            foreach (array_unique(array_filter($identifiers)) as $identifier) {
                $text = preg_replace(
                    '/(?<![\p{L}\p{N}])' . preg_quote($identifier, '/') . '(?![\p{L}\p{N}])/ui',
                    '[individual student omitted]',
                    $text
                );
            }
        }
        $text = preg_replace('/\bStudent-\d+\b/i', '[individual student omitted]', $text);
        return $text;
    }

    /**
     * Keep only aggregate class data accepted by the external chat endpoint.
     *
     * @param array $context Client context.
     * @return array Aggregate-only context.
     */
    private static function aggregate_chat_context(array $context): array {
        $coursekeys = [
        ...
        $activities = is_array($context['atividades'] ?? null) ? $context['atividades'] : [];
        foreach (array_slice($activities, 0, 60) as $activity) {
            ...
            if ($item) {
                $clean['atividades'][] = $item;
            }
        }

        $totals = is_array($context['totais'] ?? null) ? $context['totais'] : [];
        foreach ($totalskeys as $key) {
            if (isset($totals[$key]) && is_scalar($totals[$key])) {
                $clean['totais'][$key] = $totals[$key];
            }
        }
        return $clean;
    }
    ...
    /**
     * Prepare aggregate-only conversation messages for the external provider.
     *
     * @param int $courseid Course identifier.
    ...
    private static function aggregate_chat_messages(int $courseid, array $messages): array {
        ...
        foreach (array_slice($messages, -30) as $message) {
            ...
            $clean[] = [
                ...
                'content' => self::redact_students_from_chat($courseid, $content),
            ];
        }
        return $clean;
    }
    ...
    public static function get_ai_recommendation(int $courseid, string $student_name, string $prompt): array {
        ...
        try {
            ...
            \block_mwa_dashboard\ai\audit::record((int)$USER->id, $params['courseid'], 'recommendation',
                'pedagogical_recommendation', ['policy', 'individual_summary'], 'success');
        } catch (\Throwable $exception) {
            \block_mwa_dashboard\ai\audit::record((int)$USER->id, $params['courseid'], 'recommendation',
                'pedagogical_recommendation', ['policy', 'individual_summary'], 'error');
            throw $exception;
        }
    ...
    public static function chat_message(int $courseid, string $messages, string $context = '{}'): array {
        ...
        $ctx_data = self::aggregate_chat_context(json_decode($params['context'], true) ?? []);
        array_unshift($msg_list, [
            'role' => 'user',
            'category' => 'aggregate_context',
            'content' => "[AGGREGATE COURSE DATA ONLY]\n" . json_encode($ctx_data) .
                "\nAnalyse only class-level aggregates. Do not identify, infer, rank, or profile individual students. " .
                "If asked who a student is, explain that identification must be performed locally in Moodle.",
        ]);
        ...
        try {
            ...
            \block_mwa_dashboard\ai\audit::record((int)$USER->id, $params['courseid'], 'chat',
                ...
                ['aggregate_context', 'aggregate_conversation', 'activity_metrics'], 'success');
        } catch (\Throwable $exception) {
            \block_mwa_dashboard\ai\audit::record((int)$USER->id, $params['courseid'], 'chat',
                'aggregate_pedagogical_analysis',
                ['aggregate_context', 'aggregate_conversation', 'activity_metrics'], 'error');
            throw $exception;
        }
            ...
            case 'forum':
                ...
                if ($rec) {
                    ...
                    try {
                        ...
                        // Aggregated corpus for thematic summary
                        if ($all_posts) {
            ...
            case 'quiz':
                ...
                if ($rec) {
                    ...
                    if ($question_ids) {
                        ...
                        $loadedquestions = $DB->get_records_list('question', 'id', $uniquequestionids);
                        $randomcategoryids = [];
                    ...
                    if ($questions) {
                        ...
                        $q_texts = [];
                        $num = 1;
                        $displayquestions = array_slice($questions, 0, 12, true);
                        $answersbyquestion = [];
                        if ($displayquestions) {
                            $allanswers = $DB->get_records_list(
                                'question_answers', 'question', array_keys($displayquestions), 'question ASC, id ASC'
                            );
                        ...
                        foreach ($displayquestions as $q) {
                            $qtype = $qtype_labels[$q->qtype] ?? $q->qtype;
                            $text = trim(strip_tags($q->questiontext ?? ''));
                            $text = html_entity_decode($text, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                            $text = preg_replace('/\s+/', ' ', $text);
                            $text = mb_substr($text, 0, 400);
                            if (!$text) { $num++; continue; }
                            $q_block = "Q{$num} [{$qtype}]: $text";
                            // Answers/alternatives
                            try {
                                $answers = $answersbyquestion[(int)$q->id] ?? [];
                                if ($answers) {
                                    foreach ($answers as $a) {
                                        $alt = mb_substr(trim(strip_tags($a->answer ?? '')), 0, 120);
                                        if ($alt === '') continue;
                                        $mark = ((float)($a->fraction ?? 0) > 0) ? ' [CORRETA]' : '';
                                        $q_block .= "\n   - $alt$mark";
                                    }
                                }
                            } catch (\Throwable $e) {}
                        ...
                        if ($q_texts) {
                            $add_section('QUESTÕES DO QUIZ — avalie cada uma', implode("\n\n", $q_texts));
                        }
                    } else {
