...
class observer {
    ...
    /**
     * Check whether event capture is enabled for a visible course.
     *
     * A course is eligible only when it is visible and contains a visible,
     * course-scoped instance of this block. Eligibility is read directly from
     * the database so visibility changes take effect on the next event, even
     * when multiple events are processed by the same PHP execution.
     *
    ...
    private static function course_is_enabled(int $courseid): bool {
        ...
        $sql = "SELECT DISTINCT c.id
             ...
             LEFT JOIN {block_positions} bp
                   ...
                   AND bp.contextid = ctx.id
                 WHERE c.id = :courseid
                   AND c.visible = 1
                   AND (bp.id IS NULL OR bp.visible = 1)";

        return $DB->record_exists_sql($sql, [
            'contextlevel' => CONTEXT_COURSE,
            'blockname' => 'mwa_dashboard',
            'courseid' => $courseid,
        ]);
    }
    ...
    public static function store(\core\event\base $event): void {
        ...
        // Do not duplicate Moodle events for courses that are hidden or do
        // not actively expose a visible MWA Dashboard block instance.
