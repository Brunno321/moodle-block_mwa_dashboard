...
define(['block_mwa_dashboard/dashboardstore', 'core/templates',
    'block_mwa_dashboard/engagementcalc', 'core/chartjs'], function(Store, Templates, EngagementCalc, Chart) {

    'use strict';

    var window = Store.windowFacade();
    var actionCardTargets = {};
    var gaugeAnimationTimer = 0;
    var gaugeAnimationDelay = 0;
    var gaugeAnimationToken = 0;
    var latestActionCount = null;
    ...
    function studentActivityDone(row, seq, mod){
      ...
      var gradeText=String(row['act_'+seq]===undefined||row['act_'+seq]===null?'':row['act_'+seq]).trim();
      var num=parseFloat(gradeText.replace(',','.'));
      var hasPositiveGrade=gradeText!==''&&gradeText!=='-'&&!isNaN(num)&&num>0;
      if(hasCurrent){
        return mod==='forum'?current>0:(current>0||hasPositiveGrade);
      }
      return hasPositiveGrade;
    }
    function targetIsTracked(item){
      var cmid=Number(item&&item.cmid||0);
      var name=lower(item&&item.name||'');
      var matched=null;
      (Store.getConfig().activitylinks||[]).some(function(link){
        var same=(cmid&&cmidOfLog(link)===cmid)||(!cmid&&name&&lower(link&&link.name||'')===name);
    ...
    function buildActivityTargetsForStudent(student){
      ...
      var targets=Object.keys(meta).filter(function(k){return /^act_\d+$/.test(k);}).map(function(k){
        ...
        var done=studentActivityDone(row,seq,mod);
      ...
      }).filter(function(item){return item.name&&item.available&&!item.done&&targetIsTracked(item);});
      return targets;
    }
    function buildResourceTargetsForStudent(student){
      ...
      var resources={};
      var gradedCmids={};
      var gradeMeta=grades[0]&&grades[0].__mwa_type__==='activity_names'?grades[0]:null;
      if(gradeMeta){
        Object.keys(gradeMeta).forEach(function(key){
          var match=key.match(/^act_cmid_(\d+)$/);
          var gradedCmid=match?Number(gradeMeta[key]||0):0;
          if(gradedCmid)gradedCmids[gradedCmid]=true;
        });
      }
      (Store.getConfig().activitylinks||[]).forEach(function(link){
        if(link&&link.tracked===false)return;
        if(!isOpenNow(link))return;
        var name=norm(link.name||'');
        var cmid=cmidOfLog(link);
        var mod=moduleOfLog(link);
        if(mod==='label')return;
        if(!isResourceLog(link)&&(!cmid||gradedCmids[cmid]))return;
        if(!name)return;
        var key=cmid?'cmid:'+cmid:'name:'+lower(name);
    ...
    function isNeverAccessed(s){return Number(s.moodleLastAccess||0)===0 && !s.last && Number(s.interactions||0)===0;}
    function releasedGradeBelowThreshold(s){
      var row=s&&s.gradeRow||{};
      var raw=row['Course total (Grade)'];
      if(raw===null||raw===undefined||String(raw).trim()===''||String(raw).trim()==='-')raw=s&&s.grade;
      if(raw===null||raw===undefined||String(raw).trim()===''||String(raw).trim()==='-')return false;
      var earned=Number(String(raw).trim().replace(',','.'));
      var maximum=Number(row['Course full max (Grade)']||s&&s.gradeMax||0);
      var required=maximum>0?maximum*0.60:60;
      return !isNaN(earned)&&earned<required;
    }
    function hasTotalGradeAtOrAboveThreshold(s){
      var row=s&&s.gradeRow||{};
      var raw=row['Course total (Grade)'];
      if(raw===null||raw===undefined||String(raw).trim()===''||String(raw).trim()==='-')raw=s&&s.grade;
      if(raw===null||raw===undefined||String(raw).trim()===''||String(raw).trim()==='-')return false;
    ...
    function hasOverdueDelivery(s){
      var grades=state().grades||[];
      var meta=grades[0]&&grades[0].__mwa_type__==='activity_names'?grades[0]:null;
      var row=s&&s.gradeRow||null;
      if(!meta||!row)return false;
      var now=Math.floor(Date.now()/1000);
      var enrolled=Number(s.enrolmentCreated||row._enrolment_created||0);
      return Object.keys(meta).some(function(key){
        ...
        var item={cmid:Number(meta['act_cmid_'+seq]||0),name:meta[key]||''};
        if(!due||due>=now||now>due+(7*86400)||(enrolled&&enrolled>due)||(availableFrom&&availableFrom>due)||!targetIsTracked(item))return false;
        return !studentActivityDone(row,seq,lower(meta['act_module_'+seq]||''));
      });
    }
    function hasAvailableCourseContent(){
      var links=Store.getConfig().activitylinks||[];
      if(links.some(function(link){return !!link&&link.tracked!==false&&isOpenNow(link);})){return true;}
      var grades=state().grades||[];
      var meta=grades[0]&&grades[0].__mwa_type__==='activity_names'?grades[0]:null;
      ...
      return Object.keys(meta).some(function(key){
        ...
        if(!match)return false;
        var seq=match[1];
        return isOpenNow({available:meta['act_available_'+seq],availablefrom:meta['act_availablefrom_'+seq],availableuntil:meta['act_availableuntil_'+seq]});
      });
    }
    function inactiveForFollowUp(s){
      return Number(s&&s.daysWithoutAccess||0)>=7&&hasAvailableCourseContent();
    }
    function isFollowUpStudent(s){
      if(isNeverAccessed(s))return false;
      if(hasTotalGradeAtOrAboveThreshold(s))return false;
      return hasOverdueDelivery(s)||inactiveForFollowUp(s)||releasedGradeBelowThreshold(s);
    }
    ...
    function followUpSummaryHtml(list){
      var counts={overdue:0,inactive:0,grade:0};
      (list||[]).forEach(function(s){
        if(hasOverdueDelivery(s))counts.overdue++;
        if(inactiveForFollowUp(s))counts.inactive++;
    ...
    function urgentAvatarsHtml(list,kind){
      var MAX_SHOWN=5;
      ...
      html+='</div>';
      return html+(kind==='attention'?followUpSummaryHtml(list):urgentSummaryHtml(list));
    }
    function attentionSummaryHtml(items){
      var counts={reach:0,delivery:0};
      (items||[]).forEach(function(it){
        if(it.lowReach)counts.reach++;
    ...
    function actionCard(kind,label,count,desc,students,items,primary,secondary,page,insight,chartHtml,secondaryPage,tip){
      var content='';
      if(kind==='opportunity'){content=chartHtml?'<div class="ac-chart-mini">'+chartHtml+'</div>':'';}
      else if(chartHtml){content='<div class="ac-chart-mini">'+chartHtml+'</div>';}
      else if(kind==='urgent'||kind==='attention'){content=urgentAvatarsHtml(students||[],kind);}
      ...
      if(kind!=='review'&&kind!=='opportunity'){
        var filterAttr=kind==='urgent'?' data-cl-filter-target="never"':kind==='attention'?' data-cl-filter-target="followup"':'';
        secondaryBtn='<button class="ac-btn-s" data-action-page="'+esc(secondaryPage||page)+'"'+filterAttr+'>'+esc(secondary)+'</button>';
      }
      return '<div class="ac-card '+kind+'"><div class="ac-badge-row"><span class="ac-badge '+kind+'">'+esc(label)+'</span>'+helpTip(tip)+'<span class="ac-time">'+tr('updated_now')+'</span></div><div class="ac-number '+kind+'">'+esc(count)+'</div><div class="ac-desc">'+esc(desc)+'</div>'+content+'<div class="ac-btn-row">'+primaryBtn+secondaryBtn+'</div>'+(insight?'<div class="ac-insight"><svg class="mwa-ui-icon" aria-hidden="true"><use href="#mwa-icon-lightbulb"></use></svg> '+esc(insight)+'</div>':'')+'</div>';
    }
    ...
    function reviewCatalogItem(activity){
      var cmid=Number(activity&&activity.cmid||0);
      var name=lower(activity&&activity.name||'');
      var matched=null;
      (Store.getConfig().activitylinks||[]).some(function(link){
        var same=(cmid&&cmidOfLog(link)===cmid)||(!cmid&&name&&lower(link&&link.name||'')===name);
        if(same){matched=link;return true;}
      ...
      if(matched&&matched.tracked===false)return null;
      var source=activity||matched||{};
      var merged=Object.assign({},matched||{},activity||{});
      ['available','availablefrom','availableuntil','openfrom','openafter','closeafter','timeopen','timeclose','duedate'].forEach(function(key){
        if(source[key]!==undefined&&source[key]!==null&&source[key]!=='')merged[key]=source[key];
      });
      if(matched){
        merged.cmid=cmidOfLog(matched)||merged.cmid||0;
        merged.modtype=moduleOfLog(matched)||merged.modtype||'';
        merged.url=norm(matched.url||merged.url||'');
      }
      return isCurrentlyOpenItem(merged)?merged:null;
    }
    ...
    function renderRetentionCurve(){
      var canvas=$('retentionChart');if(!canvas||!Chart)return;
      var now=Date.now();
      var weekSets={};
      state().logs.forEach(function(l){
            ...
            tooltip:{callbacks:{
              ...
              label:function(item){
                ...
                var diff=prev!==null?cur-prev:null;
                 var diffStr=diff!==null?' ('+retentionChangeLabel(diff)+')':'';
                 return activeStudentsLabel(cur)+diffStr;
              }
            }}
          },
          scales:{
    ...
    function isCurrentlyOpenItem(it){
      ...
      var availableValue=lower(it.available===undefined?'':it.available);
      if(it.available===false||availableValue==='0'||availableValue==='false'||
          availability==='closed'||availability==='future'||availability==='hidden'||
          it.closed===true||it.isclosed===true||it.open===false)return false;
      var now=Math.floor(Date.now()/1000);
      var from=parseInt(it.availablefrom||it.openfrom||it.openafter||it.timeopen||0,10)||0;
      var until=parseInt(it.availableuntil||it.closeafter||it.timeclose||it.duedate||0,10)||0;
    ...
    function renderActionCenter(){
      ...
      var high=state().students.filter(isFollowUpStudent).sort(urgentSort);
      ...
      var reviewSources=state().activities.slice();
      if(window.MWAActivities&&typeof window.MWAActivities.getChatSnapshot==='function'){
        (window.MWAActivities.getChatSnapshot()||[]).forEach(function(snapshot){
          var name=norm(snapshot.nome||snapshot.name||'');
          var cmid=Number(snapshot.cmid||0);
          var exists=reviewSources.some(function(item){
            return (cmid&&Number(item.cmid||0)===cmid)||(!cmid&&name&&lower(item.name)===lower(name));
          });
          if(!name||exists)return;
          reviewSources.push({name:name,cmid:cmid,modtype:norm(snapshot.tipo||''),unique:Number(snapshot.alunosUnicos||0),accesses:Number(snapshot.acessos||0)});
        });
      }
      var reviewKnown={};
      reviewSources.forEach(function(a){reviewKnown[a.cmid?'cmid:'+a.cmid:'name:'+lower(a.name)]=true;});
      (Store.getConfig().activitylinks||[]).forEach(function(link){
        if(!link||link.tracked===false||isTextMediaArea(link))return;
        var name=norm(link.name||'');
        var cmid=cmidOfLog(link);
        var key=cmid?'cmid:'+cmid:'name:'+lower(name);
        if(!name||reviewKnown[key])return;
        reviewSources.push(Object.assign({name:name,cmid:cmid,modtype:moduleOfLog(link),unique:0,accesses:0},link));
        reviewKnown[key]=true;
      });
      var sharedLowCoverage=window.MWAActivities&&typeof window.MWAActivities.getLowPerformanceItems==='function'
        ?window.MWAActivities.getLowPerformanceItems():null;
      var lowCoverage=Array.isArray(sharedLowCoverage)?sharedLowCoverage.map(function(item){
        item.url=activityUrl(item);
        return item;
      }):reviewSources.map(reviewCatalogItem).filter(function(a){return a&&!isTextMediaArea(a)&&isCurrentlyOpenItem(a);}).map(function(a){
        var pct=Math.round(((a.unique||0)/totalStudents)*100);
        var mod=lower(a.modtype||a.module||'');
        var hasDelivery=['assign','assignment','quiz','forum','workshop','h5pactivity','scorm'].indexOf(mod)>=0;
        var dpct=hasDelivery?mwaDeliveryPct(a,mwaSubMap,totalStudents):null;
        var lowReach=pct<60;
        var lowDelivery=hasDelivery&&dpct<60;
        var worst=lowDelivery?Math.min(pct,dpct):pct;
        var item={name:a.name,pct:pct,deliveryPct:dpct,lowReach:lowReach,lowDelivery:lowDelivery,worst:worst,unique:a.unique,value:worst+'%',cmid:a.cmid||0,modtype:a.modtype||'',url:a.url||''};
        item.url=activityUrl(item);
        return item;
      }).filter(function(a){return a.lowReach||a.lowDelivery;}).sort(function(a,b){return a.worst-b.worst});
      ...
      latestActionCount=neverAccessed.length+high.length+lowCoverage.length;
    ...
    function openBulkEmail(){var rows=Array.prototype.filter.call(document.querySelectorAll('#actionFocusList .att-row'),function(r){var chk=r.querySelector('.att-chk');return chk&&chk.checked;});if(!rows.length){rows=Array.prototype.slice.call(document.querySelectorAll('#actionFocusList .att-row'));}var targets=rows.map(function(r){return {name:r.dataset.sname||'',email:r.dataset.semail||'',userid:Number(r.dataset.userid||0)};}).filter(function(t){return t.name;});if(!targets.length)return;openBulkModal(targets);}function openBulkModal(targets,preset){
      ...
      var bulkChannels={moodle:true,email:false};
      function selectedBulkType(){
        return bulkChannels.moodle&&bulkChannels.email?'both':
          (bulkChannels.email?'email':(bulkChannels.moodle?'moodle':''));
      }
      function bulkChannelPromptLabel(){
        var type=selectedBulkType();
        return type==='both'?'Moodle message and Email':
          (type==='email'?'Email (formal)':'Moodle message (friendly)');
      }
      ...
      function updateBulkTypeButtons(){
        btnMoodle.style.borderColor=bulkChannels.moodle?'var(--blue)':'';
        btnMoodle.style.color=bulkChannels.moodle?'var(--blue)':'';
        btnMoodle.style.fontWeight=bulkChannels.moodle?'900':'';
        btnEmail.style.borderColor=bulkChannels.email?'var(--green)':'';
        btnEmail.style.color=bulkChannels.email?'var(--green)':'';
        btnEmail.style.fontWeight=bulkChannels.email?'900':'';
      }
      function setBulkType(type){
        bulkChannels.moodle=type==='moodle'||type==='both';
        bulkChannels.email=type==='email'||type==='both';
        updateBulkTypeButtons();
      }
      function toggleBulkType(type){
        bulkChannels[type]=!bulkChannels[type];
        updateBulkTypeButtons();
      }
      btnMoodle.onclick=function(){toggleBulkType('moodle');};
      btnEmail.onclick=function(){toggleBulkType('email');};
...
window.toggleSelectPriority=toggleSelectPriority;window.openBulkEmail=openBulkEmail;
    ...
    function renderSemiGauge(avg,labelText){
      ...
      progress.style.strokeDasharray='0 '+CIRC;
      ...
      gaugeAnimationToken++;
      var token=gaugeAnimationToken;
      if(gaugeAnimationTimer){clearInterval(gaugeAnimationTimer);gaugeAnimationTimer=0;}
      if(gaugeAnimationDelay){clearTimeout(gaugeAnimationDelay);gaugeAnimationDelay=0;}
      function setGaugeValue(value){
        var current=Math.max(0,Math.min(pct,value));
        progress.style.strokeDasharray=((current/100)*CIRC)+' '+CIRC;
        num.textContent=Math.round(current)+'%';
      }
      gaugeAnimationDelay=setTimeout(function(){
        if(token!==gaugeAnimationToken)return;
        gaugeAnimationDelay=0;
        var started=Date.now();
        var duration=1200;
        gaugeAnimationTimer=setInterval(function(){
          if(token!==gaugeAnimationToken){clearInterval(gaugeAnimationTimer);gaugeAnimationTimer=0;return;}
          var elapsed=Math.min(1,(Date.now()-started)/duration);
          var eased=1-Math.pow(1-elapsed,3);
          setGaugeValue(pct*eased);
          if(elapsed>=1){clearInterval(gaugeAnimationTimer);gaugeAnimationTimer=0;setGaugeValue(pct);}
        },24);
      },180);
    }
    function renderEngagement(){
      ...
      var visibleUrgent=state().students.filter(isFollowUpStudent).length;
      var totalStudents=contentReachTotal();
      ...
      var needAttention=latestActionCount===null
        ?visibleNever+visibleUrgent+visibleAttention
        :latestActionCount;
    ...
    window.MWAActionCenter = {
      ...
      isFollowUpStudent: isFollowUpStudent,
