    ...
    (function () {
      ...
      function trackedTargets(d) {
        try {
          ...
          return Array.isArray(parsed) ? parsed.filter(function(item) { return item && item.name; }) : [];
        } catch (e) {
          return [];
        }
      }
      ...
      function trackedProgress(d) {
        ...
        function contextName(log) {
          return norm(log.contextodoevento || log.eventcontext || log.context || log.nomedoevento || log.action).toLowerCase();
        }
        ...
        function isResource(target) {
          ...
          if (targetType === 'activity_completion') return false;
          var mod = norm(target && (target.mod || target.modtype || target.type)).toLowerCase();
          return ['page', 'url', 'resource', 'book', 'folder', 'imscp', 'label'].indexOf(mod) >= 0;
        }
        function hasCurrentCompletion(target) {
          if (!gradeRow || !target || !target.seq || isResource(target)) return false;
          var current = parseInt(gradeRow['act_current_' + target.seq] || 0, 10) || 0;
          var value = gradeRow['act_' + target.seq];
          var numeric = parseFloat(String(value || '').replace(',', '.'));
          return current > 0 || (!isNaN(numeric) && numeric > 0);
        }
      ...
      */
      ...
      function renderInterventionsPage() {
        ...
        var filteredData = data.filter(function(d) {
          ...
          if (fStatus) {
            ...
            if (fStatus === 'awaiting' && d.status === 'sent' && st === 'pending') {
        ...
        var filtersHtml = '<div class="int2-filters-bar">'
          ...
          + '<div class="int2-filter-group int2-search-group">'
            ...
            + '<div class="int2-search-wrap">'
              ...
              + '<span class="int2-search-icon">&#128269;</span>'
            + '</div>'
          + '</div>'
        ...
        var tableHtml = filteredData.length
          ? '<table class="int2-table">'
            ...
            + '</table>'
          : '<div class="gr-empty"><div class="gr-empty-icon">&#9993;</div><p>' + esc(data.length ? tr('int_no_filter_results') : tr('int_no_data')) + '</p></div>';
        ...
        var tableHeaderHtml = '<div class="int2-table-header">'
          ...
          + '<div class="int2-table-actions">'
            + (filteredData.length
              ? '<button id="int2ExportBtn" class="int2-export-btn">&#128202; ' + tr('int_export_btn') + '</button>'
              : '')
          + '</div>'
          + '</div>';
        ...
        function snapshotBlock() {
          ...
          var definitions = {
            ...
            low: [
              ...
              ['snapshot_engagement', data.engagement === null ? null : data.engagement + '/100'],
          ...
          var gradeKpi = snapshotGrade === null ? '' : '<div class="int-snapshot-grade-kpi' + (snapshotGrade < 60 ? ' is-below-reference' : ' is-approved') + '">'
            ...
            + '</div>';
          var snapshotKpis = engagementKpi || gradeKpi ? '<div class="int-snapshot-kpi-row">' + engagementKpi + gradeKpi + '</div>' : '';
          var snapshotContent = snapshotKpis
            + '<div class="int-snapshot-concepts">'
            + '<div><span>' + esc(tr('snapshot_situation_label')) + '</span><strong>' + esc(d.snapshot_situation || d.snapshot_reason || '') + '</strong></div>'
          ...
          return '<div class="int-snapshot-launch">'
            ...
            + '<span class="int-snapshot-open-icon">📊</span><span><strong>' + esc(snapshotTitle()) + '</strong><small>' + esc(tr('snapshot_open')) + '</small></span><b>›</b>'
            + '</button>'
      ...
      function openSnapshot(id) {
        ...
        Store.renderHtml(overlay, '<section class="int-snapshot-modal" role="dialog" aria-modal="true" aria-labelledby="intSnapshotModalTitle">'
          + '<header class="int-snapshot-modal-head"><div><span>📊</span><strong id="intSnapshotModalTitle">' + esc(snapshotTitle()) + '</strong></div>'
