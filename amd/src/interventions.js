...
define(['block_mwa_dashboard/dashboardstore', 'core/templates'], function(Store, Templates) {
    ...
    (function () {
      ...
      /* module state */
      var INTERVENTIONS = [];
      function syncEvolutionReportButtons() {
        document.querySelectorAll('.cl-report-btn[data-student]').forEach(function(button) {
          var student = norm(button.getAttribute('data-student')).toLowerCase();
          button.hidden = !INTERVENTIONS.some(function(record) {
            return norm(record.student_name).toLowerCase() === student;
          });
        });
      }
      ...
      function openSendMessage(studentName, studentEmail, studentId, reason) {
        ...
        overlay.className = 'block-mwa-dashboard-app mwa-msg-overlay';
        ...
        /* Keep the message dialog above dashboard tooltips and other local
           stacking contexts. Blurring the trigger also closes a tooltip that
           may still be visible when the dialog is opened. */
      ...
      function loadInterventions(cb) {
        ...
        callAMD('block_mwa_dashboard_get_interventions', {
          courseid: courseid,
          groupid: parseInt(Store.getConfig().groupid || 0, 10)
        }, function (res) {
          try { INTERVENTIONS = JSON.parse(res.interventions || '[]'); } catch (e) { INTERVENTIONS = []; }
          syncEvolutionReportButtons();
      ...
      */
      ...
      function renderInterventionsPage() {
        ...
        var motivoHtml = motivoCards.map(function(c) {
          ...
          var percentage = totalInterventions > 0 ? Math.round((c.count / totalInterventions) * 100) : 0;
          return '<div class="int2-motivo-card' + (isActive ? ' active' : '') + '" data-bucket="' + esc(c.bucket) + '" style="'
            ...
            + '<div class="int2-motivo-head">'
              ...
              + '<div class="int2-motivo-label" style="color:' + c.color + ';">' + esc(c.label)
                + ' <span class="mwa-help-tip int2-motivo-help" tabindex="0" role="img" aria-label="' + esc(c.desc) + '" title="' + esc(c.desc) + '" data-tooltip="' + esc(c.desc) + '">?</span>'
              + '</div>'
            + '</div>'
            + '<div class="int2-motivo-count" style="color:' + c.color + ';">' + c.count + '</div>'
            + '<div class="int2-motivo-footer">' + percentage + '% ' + esc(tr('int_motivo_of_total')) + '</div>'
            + '</div>';
        }).join('');
        ...
        var tableRows = pageData.map(function(d) {
          ...
          return '<tr class="int2-row" onclick="window.MWAInterventions.toggleDetail(' + d.id + ')">'
            ...
            + '<td class="int2-td int2-td-actions">'
              + '<button class="int2-delete-direct" type="button" title="' + esc(tr('int_menu_delete')) + '" aria-label="' + esc(tr('int_menu_delete')) + '" onclick="event.stopPropagation();window.MWAInterventions.deleteIntervention(' + d.id + ')">' + icon('trash') + '</button>'
              + '<span class="int2-row-arrow" aria-hidden="true">&nearr;</span>'
            + '</td>'
            + '</tr>'
        ...
        var tableHtml = filteredData.length
          ? '<table class="int2-table">'
            ...
            + '</table>'
          : '<div class="int2-empty"><div class="int2-empty-icon">' + icon('mail') + '</div><p>' + esc(data.length ? tr('int_no_filter_results') : tr('int_no_data')) + '</p></div>';
        ...
        var tableHeaderHtml = '<div class="int2-table-header">'
          ...
          + '<div class="int2-table-actions">'
            + (filteredData.length
              ? '<button id="int2ExportBtn" class="btn-ghost cl-export-btn">' + icon('file') + ' ' + tr('cl_export_spreadsheet') + '</button>'
              : '')
          + '</div>'
          + '</div>';
      ...
      function renderDetailRow(d, effectHtml, statusHtml) {
        ...
        function snapshotBlock() {
          ...
          var definitions = {
            ...
            pending: [
              ...
              ['snapshot_oldest_pending', (data.oldestPendingDays === null || data.oldestPendingDays === undefined ? 0 : data.oldestPendingDays) + ' ' + tr('snapshot_days')]
            ],
            difficult: [
        ...
        return '<div class="int-collapse-panel" style="margin:0;border-radius:0;border:0;box-shadow:none;background:#f8fbff;">'
            + '<div class="int-collapse-top">'
              + '<div class="int-collapse-heading">'
      ...
      function toggleDetail(id) {
        ...
        function closeOverlay() {
          var overlay = document.getElementById('intDetailOverlay');
          if (!overlay) return;
          var panel = overlay.querySelector('.int-collapse-panel');
          var source = document.getElementById(overlay.dataset.sourceId || '');
          var cell = source && source.querySelector('td');
          if (panel && cell) cell.appendChild(panel);
          overlay.remove();
          document.body.classList.remove('mwa-detail-modal-open');
        }
        var current = document.getElementById('intDetailOverlay');
        if (current) {
          var same = current.dataset.sourceId === tr2.id;
          closeOverlay();
          if (same) return;
        }
        var panel = tr2.querySelector('.int-collapse-panel');
        if (!panel) return;
        var overlay = document.createElement('div');
        overlay.id = 'intDetailOverlay';
        overlay.className = 'block-mwa-dashboard-app int-record-detail-overlay';
        overlay.dataset.sourceId = tr2.id;
        document.body.appendChild(overlay);
        document.body.classList.add('mwa-detail-modal-open');
        Templates.render('block_mwa_dashboard/intervention_detail_modal', {
          title: tr('int_view_details'),
          close: tr('snapshot_close')
        }).then(function(html, js) {
          if (!document.body.contains(overlay)) return;
          Templates.replaceNodeContents(overlay, html, js);
          overlay.querySelector('.int-record-detail-modal-body').appendChild(panel);
          overlay.addEventListener('click', function(event) { if (event.target === overlay) closeOverlay(); });
          overlay.addEventListener('keydown', function(event) { if (event.key === 'Escape') closeOverlay(); });
          overlay.querySelector('.int-record-detail-modal-close').addEventListener('click', closeOverlay);
          overlay.querySelector('.int-record-detail-close').addEventListener('click', closeOverlay);
          overlay.querySelector('.int-record-detail-modal-close').focus();
        }).catch(function() {
          closeOverlay();
        });
      }
      ...
      function openProfile(el) {
        var name = el && el.getAttribute('data-sname');
        if (!name || !window.goToStudentProfile) return;
        var overlay = document.getElementById('intDetailOverlay');
        if (overlay) overlay.remove();
        document.body.classList.remove('mwa-detail-modal-open');
