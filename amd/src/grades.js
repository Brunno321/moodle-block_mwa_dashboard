    ...
    (function () {
      ...
      function parseGrades(raw) {
        ...
        return raw.slice(offset).filter(function (r) {
          ...
          return { name: name, total: total, totalmax: totalmax, email: email, pictureurl: pictureurl, actGrades: actGrades, actCurrent: actCurrent };
      ...
      function renderGrades() {
        ...
        var midHtml = isMid
          ? '<div class="gr-mid-warning"><svg class="mwa-ui-icon" aria-hidden="true"><use href="#mwa-icon-bell"></use></svg><div><strong>' + tr('gr_partial_warning_title') + '</strong> — ' + tr('gr_partial_warning_body').replace('{max}', maxG.toFixed(1)) + '</div></div>'
          : '';
        ...
        var approvedHtml = '<div class="gr-card approved" id="grCardApproved">'
          + '<div class="gr-card-head"><span class="gr-card-title" style="color:var(--green);"><svg class="mwa-ui-icon" aria-hidden="true"><use href="#mwa-icon-check"></use></svg>' + tr('gr_approved') + ' (' + approved.length + ')</span>' + tip('gr_tip_card_approved') + '<span class="gr-card-sub">≥ 60 ' + tr('gr_points') + '</span></div>'
      ...
      function showDetail(name, evt) {
        ...
        var rows = keys.map(function (k) {
          ...
          var stateClass = hasV ? ' has-grade' : ' no-grade';
          var col   = hasV ? 'var(--green)' : 'var(--red)';
      ...
      function exportXLSX() {
        ...
        var btn = document.querySelector('.gr-header-export-btn');
        ...
        if (btn) { btn.disabled = false; Store.renderHtml(btn, icon('download') + '<span>' + tr('gr_export') + '</span>'); }
