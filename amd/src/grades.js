...
define(['block_mwa_dashboard/dashboardstore'], function(Store) {
    ...
    (function () {
      ...
      function parseGrades(raw) {
        ...
        // Detecta colunas de nome — suporta pt e en
        ...
        // Detecta coluna de total — suporta formato API ('Course total (Grade)')
      ...
      function renderGrades() {
        ...
        function ofClass(n){ return n + ' ' + tr('gr_kpi_of') + ' ' + classSize + ' ' + tr('students'); }
        ...
        var kpisHtml =
          ...
          + '<button type="button" class="kpi c-green gr-kpi-filter gr-kpi-with-chip" onclick="window.MWAGrades&&window.MWAGrades.setFilter(\'approved\')">' + gradeChipWrap(gradeChips.approved) + '<div class="kpi-label">&#9989; ' + tr('gr_approved') + tip('gr_tip_kpi_approved') + '</div><div class="kpi-value">' + passRate + '%</div><div class="kpi-sub">' + esc(ofClass(approved.length)) + '</div></button>'
          + '<button type="button" class="kpi c-amber gr-kpi-filter gr-kpi-with-chip" onclick="window.MWAGrades&&window.MWAGrades.setFilter(\'inprogress\')">' + gradeChipWrap(gradeChips.inprogress) + '<div class="kpi-label">&#9203; ' + tr('gr_in_progress') + tip('gr_tip_kpi_in_progress') + '</div><div class="kpi-value">' + classPct(inProg.length) + '%</div><div class="kpi-sub">' + esc(ofClass(inProg.length)) + '</div></button>'
          + (noGrade.length > 0 ? '<button type="button" class="kpi c-purple gr-kpi-filter gr-kpi-with-chip" onclick="window.MWAGrades&&window.MWAGrades.setFilter(\'nograde\')">' + gradeChipWrap(gradeChips.nograde) + '<div class="kpi-label">&#9898; ' + tr('gr_no_grade') + tip('gr_tip_kpi_no_grade') + '</div><div class="kpi-value">' + classPct(noGrade.length) + '%</div><div class="kpi-sub">' + esc(ofClass(noGrade.length)) + '</div></button>' : '')
          + '<div class="kpi c-blue"><div class="kpi-label">' + tr('gr_avg_grade') + tip('gr_tip_avg_grade') + '</div><div class="kpi-value ' + avgClass + '">' + Math.round(avgNorm) + '</div><div class="kpi-sub">' + tr('gr_points') + '</div></div>'
          + (maxG > 0 ? '<div class="kpi c-teal"><div class="kpi-label">' + tr('gr_highest') + tip('gr_tip_highest') + '</div><div class="kpi-value ' + maxClass + '">' + maxG.toFixed(1) + '</div><div class="kpi-sub">' + tr('gr_points') + '</div></div>' : '')
          + '</div>';
    
        // ── Aviso de semestre parcial ──
        var midHtml = isMid
        ...
        var filterHtml =
          ...
          + '<span class="gr-filter-label">' + esc(tr('ev_filter_label')) + '</span>'
          + '<input type="hidden" id="grFilter" value="all">'
          + '<div class="gr-filter-tabs" id="grFilterTabs">'
            + '<button type="button" class="gr-tab active" data-gr-filter="all" onclick="window.MWAGrades&&window.MWAGrades.setFilter(\'all\')">' + esc(tr('ev_all')) + '</button>'
            + '<button type="button" class="gr-tab" data-gr-filter="approved" onclick="window.MWAGrades&&window.MWAGrades.setFilter(\'approved\')"><span class="gr-tab-dot green"></span>' + esc(tr('gr_approved')) + '</button>'
            + '<button type="button" class="gr-tab" data-gr-filter="inprogress" onclick="window.MWAGrades&&window.MWAGrades.setFilter(\'inprogress\')"><span class="gr-tab-dot amber"></span>' + esc(tr('gr_in_progress')) + '</button>'
            + '<button type="button" class="gr-tab" data-gr-filter="nograde" onclick="window.MWAGrades&&window.MWAGrades.setFilter(\'nograde\')"><span class="gr-tab-dot muted"></span>' + esc(tr('gr_no_grade')) + '</button>'
          + '</div>'
          + '<div class="gr-filter-actions">'
            + '<label class="gr-search-wrap"><span class="gr-search-icon" aria-hidden="true">&#128269;</span><input id="grSearch" type="text" class="gr-search" placeholder="' + esc(tr('gr_search_placeholder')) + '" oninput="window.MWAGrades&&window.MWAGrades.filter()"></label>'
