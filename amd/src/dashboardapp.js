...
define([
    ...
    'block_mwa_dashboard/engagement',
    'block_mwa_dashboard/content',
    'block_mwa_dashboard/personalised',
    'block_mwa_dashboard/chat',
    'core/templates'
], function(Store, ActionCenter, Alerts, ClassList, Profile, Activities, Grades, Interventions, Engagement, Content, Personalised, Chat, Templates) {
    ...
    var init = function(config) {
        ...
        document.addEventListener('click', function(e){
          // Button with data-bulk-kind only (no data-action-page) → opens modal
          ...
          // Navigation buttons (data-action-page, without data-bulk-kind)
          var btn = e.target.closest('[data-action-page]');
          if (btn) {
            e.preventDefault();
            showPage(btn.getAttribute('data-action-page'));
          }
        });
        ...
        function activityName(log){return norm(log.contextodoevento)||norm(log.context)||norm(log._modtype)||tr('unknown_activity','Unknown activity')}
        function componentName(log){return norm(log.componente)||norm(log.component)||norm(log._modtype)||tr('other','Other')}
        function buildModel(){var by={};var activitiesMap={};state.logs.forEach(function(l){var name=norm(l.nomecompleto)||tr('unknown_student','Unknown student');var email=norm(l.email);var key=studentKey(name,email);if(!by[key])by[key]={name:name,email:email,interactions:0,contexts:{},submissions:0,last:null,grade:null,score:0};by[key].interactions++;var a=activityName(l);by[key].contexts[a]=true;activitiesMap[a]=activitiesMap[a]||{name:a,type:componentName(l),accesses:0,students:{}};activitiesMap[a].accesses++;activitiesMap[a].students[key]=true;if(isSubmission(l))by[key].submissions++;var d=parseDate(l);if(d&&(!by[key].last||d>by[key].last))by[key].last=d});state.grades.forEach(function(g){var email=gradeEmail(g), name=gradeName(g);var key=studentKey(name,email);if(!by[key])by[key]={name:name||email||tr('unknown_student','Unknown student'),email:email,interactions:0,contexts:{},submissions:0,last:null,grade:null,score:0};var gt=gradeTotal(g);if(gt!==null)by[key].grade=gt;by[key].gradeRow=g});var totalActivities=Math.max(1,Object.keys(activitiesMap).length);state.students=Object.keys(by).map(function(k){var s=by[k];var coverage=Object.keys(s.contexts).length/totalActivities*100;var accessScore=Math.min(100,s.interactions*10);var submitScore=s.submissions>0?100:0;var grade=(s.grade===null?null:s.grade);var score;if(grade!==null){score=Math.round(grade*.5+coverage*.3+submitScore*.2)}else{score=Math.round(accessScore*.6+coverage*.4)}s.coverage=Math.round(coverage);s.score=score;s.risk=score<40?'High':score<70?'Medium':'Low';return s}).sort(function(a,b){return a.score-b.score});state.activities=Object.keys(activitiesMap).map(function(k){var a=activitiesMap[k];a.unique=Object.keys(a.students).length;return a}).sort(function(a,b){return b.accesses-a.accesses})}
        /* ── Delta chip ▲▼▬ reusable across all tabs ── */
        window.MWADeltaChip = function(cur, prv, invertColors) {
          ...
          var arrow  = d === 0 ? '▬' : d > 0 ? '▲' : '▼';
          var sign   = d > 0 ? '+' : '';
          return '<span style="background:'+bg+';color:'+color+';font-size:.65rem;font-weight:800;padding:2px 7px;border-radius:99px;float:right;margin-left:4px;">'+arrow+' '+sign+d+'</span>';
        };

        /* ── Split logs into recent half vs earlier half ── */
        ...
        /* ── Renderiza um KPI inteligente ── */
        function kpiSmart(label, value, sub, delta, invertDelta) {
          // delta: number or null
          var chipHtml = '';
          if (delta !== null && delta !== undefined && !isNaN(Number(delta))) {
            var pos = invertDelta ? delta < 0 : delta > 0; // invertido para "em risco"
            ...
            var sign    = delta > 0 ? '+' : '';
            chipHtml = '<span style="background:'+chipBg+';color:'+chipClr+';font-size:.68rem;font-weight:800;padding:2px 8px;border-radius:99px;letter-spacing:.02em;">'+arrow+' '+sign+delta+'</span>';
          }
          return '<div class="kpi">'            + '<div class="kpi-label" style="display:flex;justify-content:space-between;align-items:center;">'            + '<span>'+label+'</span>'+chipHtml            + '</div>'            + '<div class="kpi-value">'+value+'</div>'            + '<div class="kpi-sub">'+sub+'</div>'            + '</div>';
        }
        ...
        function renderKPIs(){
          ...
          // Compare recent half vs metade anterior dos logs
          ...
          // Grade delta: average grade of active students in recent vs earlier period
          var dGrade = null;
          ...
          Store.renderHtml($('kpis'), kpiSmart(tr('students_in_log','Estudantes no log'),     state.students.length, tr('active_students','estudantes ativos'),   dStudents, false)
          + kpiSmart(tr('total_interactions','Total de interações'), state.logs.length,     tr('moodle_events','eventos do Moodle'),     dLogs,     false)
          + kpiSmart(tr('average_student','Média por estudante'),    avgInt,                tr('interactions','interações'),             dAvgInt,   false)
          + kpiSmart(tr('at_risk','Em risco'),                       atRisk,                tr('score_below_40','score abaixo de 40'),   dRisk,     true)
          + (avgGrade!==null ? kpiSmart(tr('grade_average','Média das notas'), avgGrade, tr('of_100_points','de 100 pontos'), dGrade, false) : ''));
        }
        
        function renderChart(){var SKIP={System:1,Login:1,system:1,login:1,outro:1,Outro:1};var counts={};state.logs.forEach(function(l){var c=componentName(l);if(SKIP[c])return;counts[c]=(counts[c]||0)+1});var labels=Object.keys(counts);var data=labels.map(function(k){return counts[k]});var ctx=$('eventChart');if(!ctx||!window.Chart)return; if(eventChart)eventChart.destroy();eventChart=new Chart(ctx,{type:'doughnut',data:{labels:labels,datasets:[{data:data,backgroundColor:COLORS,borderWidth:0,hoverOffset:4}]},options:{cutout:'62%',plugins:{legend:{position:'bottom',labels:{usePointStyle:false,boxWidth:12,padding:14,font:{size:12}}}},maintainAspectRatio:false}})}
        ...
        function renderHeatmap() {
          var DAYS = [tr('sun','Dom'),tr('mon','Seg'),tr('tue','Ter'),tr('wed','Qua'),
                      tr('thu','Qui'),tr('fri','Sex'),tr('sat','Sáb')];
          ...
          /* ── preenche selects com tr() (garante i18n ao trocar idioma) ── */
          if (selStudent && selStudent.options.length === 0) {
            var optAll = document.createElement('option');
            optAll.value = ''; optAll.textContent = '👥 ' + tr('hm_filter_all_students','Turma toda');
          ...
          if (selMod && selMod.options.length === 0) {
            [
              ...
              {v:'quiz',        l:'❓ '+tr('hm_filter_quiz','Questionários')},
              {v:'forum',       l:'💬 '+tr('hm_filter_forum','Fóruns')},
              {v:'resource',    l:'📄 '+tr('hm_filter_resource','Arquivos')},
              {v:'url',         l:'🔗 '+tr('hm_filter_url','URLs')},
              {v:'page',        l:'📖 '+tr('hm_filter_page','Páginas')},
              {v:'h5pactivity', l:'🎮 '+tr('hm_filter_h5p','H5P')},
              {v:'scorm',       l:'📦 '+tr('hm_filter_scorm','SCORM')},
              {v:'video',       l:'🎥 '+tr('hm_filter_video','Vídeos')}
            ].forEach(function(o){
              var opt=document.createElement('option');
          ...
          if (selMode && selMode.options.length === 0) {
            [{v:'access',l:'🟢 '+tr('hm_mode_access','Acessos')},
          ...
          var logs = state.logs.filter(function(l) {
            ...
            if (modFilter && (norm(l._modtype)||'').toLowerCase().indexOf(modFilter) < 0) return false;
            return true;
          });

          /* ── grid principal ── */
          var grid = {};      // day:hour → count
          var lastGrid = {};  // day:hour → count of last accesses (dropout mode)
          var weekTrend = [{},{}]; // [semana anterior, esta semana]
          var now = new Date();
          ...
          logs.forEach(function(l) {
            ...
            if (mode === 'access') {
              ...
              /* dropout: count only last access per student per slot */
          ...
          /* modo abandono: converte lastGrid → grid */
          if (mode === 'dropout') {
          ...
          /* ── deadlines detectados (prazos: horas com pico isolado) ── */
          var deadlines = {};
          ...
          for (var d3=0; d3<7; d3++) {
            ...
            for (var h3=0; h3<24; h3++) {
              ...
              var label = v3 ? (v3 + trend + deadline) : '';
              var tip = DAYS[d3] + ' ' + h3 + 'h: ' + v3 + ' '+(v3===1?tr('hm_access_count','{n} acesso').replace('{n}',''):tr('hm_accesses_count','{n} acessos').replace('{n}','')) +
                        (trend === ' ▲' ? ' (+'+tr('hm_trend_up','tendência de alta')+')' : trend === ' ▼' ? ' (-'+tr('hm_trend_down','tendência de baixa')+')' : '') +
                        (deadline ? ' — '+tr('hm_possible_deadline','possível prazo!') : '');
              html += '<div class="' + cls + (deadlines[d3+':'+h3]?' hm-deadline':'') +
          ...
          /* Best time slot (highest sum across all days) */
          ...
          /* Best 2-hour window for intervention */
          ...
          if (kpiBox) {
            Store.renderHtml(kpiBox, '<div class="hm-kpi"><div class="hm-kpi-val">' + pctAfter + '%</div>'
              + '<div class="hm-kpi-lbl">'+tr('hm_kpi_after_hours','acessos após 18h')+'</div></div>'
              + '<div class="hm-kpi"><div class="hm-kpi-val">' + bestH + 'h</div>'
              + '<div class="hm-kpi-lbl">'+tr('hm_kpi_peak_hour','horário de pico')+'</div></div>'
              + '<div class="hm-kpi"><div class="hm-kpi-val">' + total + '</div>'
              + '<div class="hm-kpi-lbl">'+tr('hm_kpi_filtered','acessos filtrados')+'</div></div>'
              + '<div class="hm-kpi"><div class="hm-kpi-val">' + state.logs.length + '</div>'
              + '<div class="hm-kpi-lbl">'+tr('hm_kpi_total_logs','total de logs')+'</div></div>');
          }
          ...
          if (total > 0) {
            insights.push('✓ '+tr('hm_peak_insight','Pico: {day} às {hour}h').replace('{day}',DAYS[peakDay]).replace('{hour}',String(bestH)) + DAYS[peakDay] + ' às ' + bestH + 'h</strong>.');
            insights.push('✓ '+tr('hm_after18_insight','{pct}% dos acessos ocorreram após as 18h.').replace('{pct}',String(pctAfter)));
            insights.push('✓ '+tr('hm_besttime_insight','Melhor horário: {start}h–{end}h.').replace('{start}',String(best2hStart)).replace('{end}',String(best2hStart+2)));
            /* Detecta se alunos acessam perto de prazos */
            var deadlineCount = Object.keys(deadlines).length;
            if (deadlineCount > 0) {
              insights.push('⚠️ '+tr('hm_deadline_insight','Detectados {n} possíveis prazos com pico de acesso (células 🔴).').replace('{n}',String(deadlineCount)));
            }
            /* Overall trend */
            ...
            if (prevTotal > 0) {
              ...
              insights.push((chg>=0?'▲':'▼')+' '+tr('hm_trend_insight','Tendência semanal: {pct}% em relação à semana anterior.').replace('{pct}',(chg>=0?'+':'')+String(chg)));
            }
          } else {
            insights.push(tr('hm_no_data','Nenhum dado para o filtro selecionado.'));
          }
          ...
          /* ── Grade × time-slot correlation ── */
          var hourGrades = {};
          state.students.forEach(function(s) {
            if (s.grade === null) return;
            /* hora favorita deste aluno */
            var uHours = Array(24).fill(0);
            state.logs.filter(function(l){ return norm(l.nomecompleto) === norm(s.name); })
              .forEach(function(l){ var d=parseDate(l); if(d) uHours[d.getHours()]++; });
            var uBest = uHours.indexOf(Math.max.apply(null,uHours));
            var bucket = uBest<12?tr('hm_morning','Manhã (6h–12h)'):uBest<18?tr('hm_afternoon','Tarde (12h–18h)'):tr('hm_evening','Noite (18h–24h)');
          ...
          Object.keys(hourGrades).sort().forEach(function(bucket) {
            ...
            gcHTML += '<div style="display:flex;justify-content:space-between;align-items:center;'
              ...
              + tr('hm_grade_avg','Média') + ' ' + avg + '</strong></div>';
          });
          ...
          if (btBox && total > 0) {
            ...
            Store.renderHtml(btBox, '<div style="font-size:2rem;font-weight:900;color:var(--blue);letter-spacing:-.04em;">'
              + best2hStart + 'h–' + (best2hStart+2) + 'h</div>'
              + '<p style="margin:6px 0 0;font-size:.82rem;color:var(--muted);">'
              + '<strong>' + pct2h + '%</strong> '+tr('hm_besttime_desc','dos alunos costumam acessar nesse período.')+'<br>'
              + tr('hm_besttime_tip','Envie mensagens antes desse horário para maior chance de leitura.')+'</p>');
          } else if (btBox) {
            Store.renderHtml(btBox, '<p style="color:var(--muted);font-size:.82rem;">'+tr('hm_no_data_simple','Sem dados.')+'</p>');
          }
        ...
        function renderReport(){var el=$('reportBody');if(!el)return;var risk=state.students.filter(function(s){return s.score<40}).length;var medium=state.students.filter(function(s){return s.score>=40&&s.score<70}).length;var high=state.students.filter(function(s){return s.score>=70}).length;Store.renderHtml(el, '<p><strong>'+tr('total_students','Total students')+':</strong> '+state.students.length+'</p><p><strong>'+tr('total_interactions','Total interactions')+':</strong> '+state.logs.length+'</p><p><strong>'+tr('activities_detected','Activities detected')+':</strong> '+state.activities.length+'</p><p><strong>'+tr('engagement_groups','Engagement groups')+':</strong> '+risk+' '+tr('low','low')+', '+medium+' '+tr('average','average')+', '+high+' '+tr('high','high')+'.</p><p class=\"muted\">'+tr('report_note','This report uses stable Moodle data fields.')+'</p>');}
        function renderAll(){buildModel();dashboard.state=state;Store.setDashboard(dashboard);renderKPIs();if(Modules.ActionCenter){Modules.ActionCenter.renderEngagement();Modules.ActionCenter.render();}if(Engagement){Engagement.render();}if(Content){Content.render();}if(Personalised){Personalised.render();}if(Chat){Chat.render();}renderChart();renderTables();renderAlerts();renderHeatmap();renderReport();if(Modules.ClassList)Modules.ClassList.render();if(Modules.Profile)Modules.Profile.render();if(Modules.Activities)Modules.Activities.render();if(Modules.Grades)Modules.Grades.render();if(Modules.Interventions)Modules.Interventions.render();$('loading').style.display='none'}
        function esc(v){return String(v===undefined||v===null?'':v).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})}
        function showPage(p){document.querySelectorAll('.page').forEach(function(x){x.classList.remove('active')});var page=$('page-'+p);if(page)page.classList.add('active');document.querySelectorAll('.nav-item').forEach(function(x){x.classList.toggle('active',x.getAttribute('data-page')===p)});if(p==='alerts'&&Modules.Alerts&&state.logs.length){Modules.Alerts.reset();Modules.Alerts.render();}if(p==='classlist'&&Modules.ClassList&&state.logs.length){Modules.ClassList.render();}if(p==='studentprofile'&&Modules.Profile&&state.logs.length){Modules.Profile.render();}if(p==='activities'&&Modules.Activities&&state.logs.length){Modules.Activities.render();}if(p==='grades'&&Modules.Grades){Modules.Grades.render();}if(p==='interventions'&&Modules.Interventions){Modules.Interventions.render();}if(p==='engagement'&&state.logs.length&&Engagement){Engagement.render();}if(p==='personalised'&&state.logs.length&&Personalised){Personalised.render();}if(p==='chat'&&Chat){Chat.render();}if(p==='contentanalysis'&&Content){var ctEl=document.getElementById('prescriptiveContent');renderInlineStatus(ctEl,'⏳ '+tr('ct_updated','Updating...'));var _cfg=typeof Store!=='undefined'&&Store.getConfig?Store.getConfig():{};var _cid=parseInt(_cfg.courseid||0,10);if(!_cid){var _m=(window.location.search||'').match(/[?&]id=(\d+)/);if(_m)_cid=parseInt(_m[1],10);}if(_cid&&typeof window.MWAReloadData==='function'){window.MWAReloadData().then(function(){Content.render();}).catch(function(){Content.render();});}else{Content.render();}var periodBar=document.getElementById('contentPeriodBar');if(periodBar&&!periodBar.dataset.ctInit){periodBar.dataset.ctInit='1';periodBar.addEventListener('click',function(ev){var btn=ev.target.closest('[data-ct-period]');if(btn)Content.setPeriod(Number(btn.getAttribute('data-ct-period')),btn);});}}if(p==='heatmap'&&state.logs.length){renderHeatmap();['heatmapStudentSel','heatmapModSel','heatmapModeSel'].forEach(function(id){var el=document.getElementById(id);if(el&&!el.dataset.hmInit){el.dataset.hmInit='1';el.addEventListener('change',renderHeatmap);}});}}
