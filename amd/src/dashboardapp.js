...
define([
    ...
    var init = function(config) {
        ...
        function renderKPIs(){
          ...
          var activeCourseStudents = Object.keys(logStudentNames).length;
          var avgInt = activeCourseStudents ? Math.round((totalStudentInteractions / activeCourseStudents) * 10) / 10 : 0;
          ...
          Store.renderHtml($('kpis'), kpiSmart(tr('students_in_log','Estudantes ativos'),     state.students.length, tr('students_loaded','estudantes ativos'),   dActiveStudents, false, tr('ac_tip_students_in_log','Quantidade de estudantes matriculados carregados do Moodle para este curso.'), participantsUrl ? {url: participantsUrl, newWindow: true, icon:'&#128101;', theme:'blue'} : {icon:'&#128101;', theme:'blue'})
          + kpiSmart(tr('active_course_students','Estudante/Interação'), activeCourseStudents, tr('with_course_activity','com interações'), dStudents, false, tr('ac_tip_active_course_students','Quantidade de estudantes que fizeram pelo menos um acesso ou interação registrada no curso.'), {page:'classlist', classFilter:'interactions', icon:'&#128260;', theme:'teal', nonDecreasing:true})
          + kpiSmart(tr('total_interactions','Total de interações'), totalStudentInteractions,     tr('moodle_events','eventos do Moodle'),     dLogs,     false, tr('ac_tip_total_interactions'), {icon:'&#128172;', theme:'green', nonDecreasing:true})
          + kpiSmart(tr('average_student','Média por estudante'),    avgInt,                tr('interactions','interações'),             dAvgInt,   false, tr('ac_tip_average_student'), {icon:'&#128200;', theme:'amber'})
          + (avgGrade!==null ? kpiSmart(tr('grade_average'), avgGrade.toFixed(1), tr('of_100_points','de 100 pontos'), dGrade, false, tr('ac_tip_grade_average'), {page:'grades', icon:'&#127891;', theme:'purple'}) : ''));
        }
        ...
        function renderHeatmap() {
          ...
          if (kpiBox) {
            Store.renderHtml(kpiBox, '<button type="button" class="hm-kpi hm-kpi-amber" data-hm-kpi="peak" data-hm-key="' + esc(peakKey) + '"><div class="hm-kpi-head"><span class="hm-kpi-icon" aria-hidden="true">&#9200;</span><div class="hm-kpi-lbl">'+tr('hm_kpi_peak_hour','horario de pico')+'</div></div><div class="hm-kpi-val">' + bestH + 'h</div></button>'
              + '<button type="button" class="hm-kpi hm-kpi-blue" data-hm-kpi="filtered"><div class="hm-kpi-head"><span class="hm-kpi-icon" aria-hidden="true">&#128269;</span><div class="hm-kpi-lbl">'+tr('hm_kpi_filtered')+'</div></div><div class="hm-kpi-val">' + total + '</div></button>'
              + '<button type="button" class="hm-kpi hm-kpi-teal" data-hm-kpi="all"><div class="hm-kpi-head"><span class="hm-kpi-icon" aria-hidden="true">&#128202;</span><div class="hm-kpi-lbl">'+tr('hm_kpi_total_logs','total de logs')+'</div></div><div class="hm-kpi-val">' + state.logs.length + '</div></button>');
          }
        ...
        function showPage(p){document.querySelectorAll('.page').forEach(function(x){x.classList.remove('active')});var page=$('page-'+p);if(page)page.classList.add('active');document.querySelectorAll('.nav-item').forEach(function(x){x.classList.toggle('active',x.getAttribute('data-page')===p)});if(p==='alerts'&&Modules.Alerts&&state.logs.length){Modules.Alerts.reset();Modules.Alerts.render();}if(p==='classlist'&&Modules.ClassList&&state.logs.length){Modules.ClassList.render();}if(p==='studentprofile'&&Modules.Profile&&state.logs.length){if(typeof window.MWAReloadData==='function'&&!window.MWAProfileReloading){window.MWAProfileReloading=true;window.MWAReloadData().then(function(){Modules.Profile.render();}).catch(function(){Modules.Profile.render();}).then(function(){window.MWAProfileReloading=false;});}else{Modules.Profile.render();}}if(p==='activities'&&Modules.Activities&&state.logs.length){Modules.Activities.render();}if(p==='grades'&&Modules.Grades){Modules.Grades.render();}if(p==='interventions'&&Modules.Interventions){if(typeof window.MWAReloadData==='function'&&!window.MWAInterventionsReloading){window.MWAInterventionsReloading=true;window.MWAReloadData().then(function(){Modules.Interventions.render();}).catch(function(){Modules.Interventions.render();}).then(function(){window.MWAInterventionsReloading=false;});}else{Modules.Interventions.render();}}if(p==='teacherfeedback'&&Modules.TeacherFeedback){Modules.TeacherFeedback.render();}if(p==='chat'&&Chat){Chat.render();}if(p==='heatmap'&&state.logs.length){renderHeatmap();['heatmapStudentSel','heatmapModSel','heatmapModeSel'].forEach(function(id){var el=document.getElementById(id);if(el&&!el.dataset.hmInit){el.dataset.hmInit='1';el.addEventListener('change',renderHeatmap);}});}}
