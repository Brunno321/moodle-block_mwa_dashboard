    ...
    (function () {
      ...
      function buildSmartAlertData() {
        ...
        var neverAccessed = getStudents().filter(function (s) {
          return Number(s.moodleLastAccess || 0) === 0 && !s.last && Number(s.interactions || 0) === 0;
        }).map(function (s) {
        ...
        /* Alert 4 — Symbolic access (≤2 active days, after the third observed calendar day). */
        var symbolic = [];
        keys.forEach(function (k) {
          var s      = byStudent[k];
          var evs    = s.logs.filter(function (r) { return r.date; }).sort(function (a, b) { return a.date - b.date; });
          if (!evs.length) return;
          var daySet = new Set(evs.map(function (r) { return r.date.toDateString(); }));
          var firstDay = Date.UTC(evs[0].date.getFullYear(), evs[0].date.getMonth(), evs[0].date.getDate());
          var now = new Date();
          var today = Date.UTC(now.getFullYear(), now.getMonth(), now.getDate());
          var observedCalendarDays = Math.floor((today - firstDay) / 86400000) + 1;
          if (observedCalendarDays >= 3 && daySet.size <= 2)
      ...
      function renderAlertKPIs(d) {
        ...
        var kpis = [
          { key: 'saw', cls: 'c-rose',   icon: '<svg class="mwa-ui-icon"><use href="#mwa-icon-eye"></use></svg>',  lk: 'alert_kpi_viu_label',      dk: 'alert_kpi_viu_delta',      tk: 'alert_kpi_viu_tip',      val: d.sawNotSubmit.length },
          { key: 'drop', cls: 'c-amber',  icon: '<svg class="mwa-ui-icon"><use href="#mwa-icon-trend-down"></use></svg>', lk: 'alert_kpi_queda_label',    dk: 'alert_kpi_queda_delta',    tk: 'alert_kpi_queda_tip',    val: d.sharpDrop.length    },
          { key: 'ghost', cls: 'c-purple', icon: '<svg class="mwa-ui-icon"><use href="#mwa-icon-user"></use></svg>', lk: 'alert_kpi_fantasma_label', dk: 'alert_kpi_fantasma_delta', tk: 'alert_kpi_fantasma_tip', val: d.ghosts.length       },
          { key: 'symbolic', cls: 'c-blue',   icon: '<svg class="mwa-ui-icon"><use href="#mwa-icon-bolt"></use></svg>', lk: 'alert_kpi_simbol_label',   dk: 'alert_kpi_simbol_delta',   tk: 'alert_kpi_simbol_tip',   val: d.symbolic.length     },
          { key: 'reactivated', cls: 'c-teal',   icon: '<svg class="mwa-ui-icon"><use href="#mwa-icon-refresh"></use></svg>', lk: 'alert_kpi_reat_label',     dk: 'alert_kpi_reat_delta',     tk: 'alert_kpi_reat_tip',     val: d.reactivated.length  },
          { key: 'never', cls: 'c-red',    icon: '<svg class="mwa-ui-icon"><use href="#mwa-icon-ban"></use></svg>', lk: 'alert_kpi_never_label',    dk: 'alert_kpi_never_delta',    tk: 'alert_kpi_never_tip',    val: d.neverAccessed.length },
        ];
      ...
      ════════════════════════════════════════════ */
      function saBlock(cls, iconMarkup, titleKey, tipKey, students, extraFn) {
        ...
        return '<div class="smart-alert ' + cls + '" id="' + id + '">'
          + '<div class="sa-icon">' + iconMarkup + '</div>'
          + '<div class="sa-body">'
            ...
            + '</div>'
            + '<div class="sa-tip">' + icon('lightbulb') + ' ' + esc(tr(tipKey)) + '</div>'
            + '<div class="sa-actions">'
              + (students.length
                  ? '<button class="sa-btn-primary" data-sa-bulk="1">' + icon('mail') + ' ' + esc(tr('alert_email_all')) + '</button>'
                  : '')
            + '</div>'
          + '</div>'
          + '</div>';
      ...
      function renderSmartAlerts(badgeOnly) {
        ...
        if ((SA_FILTER === 'all' || SA_FILTER === 'never') && d.neverAccessed.length)
          blocks.push(saBlock('sa-danger', icon('ban'), 'alert_block_never_title', 'alert_block_never_tip', d.neverAccessed, null));
    
        if ((SA_FILTER === 'all' || SA_FILTER === 'saw') && d.sawNotSubmit.length)
          blocks.push(saBlock('sa-rose', icon('eye'), 'alert_block_viu_title', 'alert_block_viu_tip', d.sawNotSubmit,
            function (s) { return (s.activities || []).slice(0, 3).join(', '); }));
    
        if ((SA_FILTER === 'all' || SA_FILTER === 'drop') && d.sharpDrop.length)
          blocks.push(saBlock('sa-warning', icon('trend-down'), 'alert_block_queda_title', 'alert_block_queda_tip', d.sharpDrop,
            function (s) { return trN('alert_extra_queda', s.drop); }));
    
        if ((SA_FILTER === 'all' || SA_FILTER === 'symbolic') && d.symbolic.length)
          blocks.push(saBlock('sa-info', icon('bolt'), 'alert_block_simbol_title','alert_block_simbol_tip', d.symbolic,
            function (s) { return trN('alert_extra_dias', s.days); }));
    
        if ((SA_FILTER === 'all' || SA_FILTER === 'ghost') && d.ghosts.length)
          blocks.push(saBlock('sa-purple', icon('user'), 'alert_block_ghost_title', 'alert_block_ghost_tip', d.ghosts,
            function (s) { return trN('alert_extra_sumiu', s.daysSinceLast); }));
    
        if ((SA_FILTER === 'all' || SA_FILTER === 'reactivated') && d.reactivated.length)
          blocks.push(saBlock('sa-teal', icon('refresh'), 'alert_block_reat_title', 'alert_block_reat_tip', d.reactivated,
        ...
        Store.renderHtml(list, blocks.length
          ...
          : '<div class="sa-empty"><div class="sa-empty-icon">' + icon('check') + '</div><p>' + esc(tr('alerts_no_data')) + '</p></div>');
