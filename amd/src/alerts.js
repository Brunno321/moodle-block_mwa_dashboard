...
define(['block_mwa_dashboard/dashboardstore'], function(Store) {
    ...
    (function () {
      ...
      /*
       * tr() lê Store.getStrings() diretamente — o mesmo objeto que o PHP injeta
       * via dashboard_renderer.php. Isso garante que a tradução funciona mesmo que
       * window.MWADashboard ainda não exista quando o script é carregado.
       * Idêntico ao padrão do actioncenter.js (ctx().tr aponta para esta mesma lógica).
       */
      function tr(key) {
      ...
      /* substitui {n} na string traduzida */
      function trN(key, n) {
      ...
      function groupByStudent() {
        ...
        getLogs().forEach(function (r) {
          var name = norm(r.nomecompleto) || tr('unknown_student');
          var key  = lower(norm(r.email)) || lower(name);
          if (!byStudent[key]) byStudent[key] = { name: name, email: norm(r.email), logs: [] };
      ...
      ════════════════════════════════════════════ */
      var SA_DATA = null;
      var SA_STUDENTS_MAP = {}; // id_card → array de students
      ...
      function buildSmartAlertData() {
        ...
        keys.forEach(function (k) {
          var s   = byStudent[k];
          var evs = s.logs;
          var saw = evs.some(function (e) {
            var ev   = lower(norm(e.nomedoevento));
            var comp = lower(norm(e.componente));
            // only count "viewed" events for assignments
            return (comp === 'assignment' || comp === 'tarefa') &&
                   (ev.indexOf('course module viewed') !== -1 ||
                    ev.indexOf('submission status viewed') !== -1 ||
                    ev.indexOf('submission_status_viewed') !== -1 ||
                    ev.indexOf('viewed') !== -1);
          });
          var submitted = evs.some(function (e) {
            var ev   = lower(norm(e.nomedoevento));
            var comp = lower(norm(e.componente));
            return (comp === 'assignment' || comp === 'tarefa') &&
          ...
          if (saw && !submitted) sawNotSubmit.push({ name: s.name, email: s.email });
        });
        ...
        SA_DATA = { built: true, sawNotSubmit: sawNotSubmit, sharpDrop: sharpDrop,
                    ghosts: ghosts, symbolic: symbolic, nightOwls: nightOwls, reactivated: reactivated };
        return SA_DATA;
      }
      ...
      function renderAlertKPIs(d) {
        ...
        var kpis = [
          { cls: 'c-red',    icon: '👁',  lk: 'alert_kpi_viu_label',      dk: 'alert_kpi_viu_delta',      tk: 'alert_kpi_viu_tip',      val: d.sawNotSubmit.length },
          { cls: 'c-amber',  icon: '📉', lk: 'alert_kpi_queda_label',    dk: 'alert_kpi_queda_delta',    tk: 'alert_kpi_queda_tip',    val: d.sharpDrop.length    },
          { cls: 'c-purple', icon: '👻', lk: 'alert_kpi_fantasma_label', dk: 'alert_kpi_fantasma_delta', tk: 'alert_kpi_fantasma_tip', val: d.ghosts.length       },
          { cls: 'c-blue',   icon: '⚡', lk: 'alert_kpi_simbol_label',   dk: 'alert_kpi_simbol_delta',   tk: 'alert_kpi_simbol_tip',   val: d.symbolic.length     },
          { cls: 'c-teal',   icon: '🔄', lk: 'alert_kpi_reat_label',     dk: 'alert_kpi_reat_delta',     tk: 'alert_kpi_reat_tip',     val: d.reactivated.length  },
          { cls: 'c-green',  icon: '🌙', lk: 'alert_kpi_madru_label',    dk: 'alert_kpi_madru_delta',    tk: 'alert_kpi_madru_tip',    val: d.nightOwls.length    },
        ];
        ...
        var kpiDeltas=[alDelta(d.sawNotSubmit,true),alDelta(d.sharpDrop,true),alDelta(d.ghosts,true),alDelta(d.symbolic,true),alDelta(d.reactivated,false),alDelta(d.nightOwls,false)];
        Store.renderHtml(container, kpis.map(function (kpi, i) {
          return '<div class="sa-kpi ' + kpi.cls + '">'
            + '<div class="sa-kpi-label">' + kpi.icon + ' ' + esc(tr(kpi.lk)) + (kpiDeltas[i]||'') + '</div>'
      ...
      function saBlock(cls, icon, titleKey, tipKey, students, extraFn) {
        ...
        SA_STUDENTS_MAP[id] = students; // guardar para uso no click de mensagem
        ...
        var fmt = function (s) {
          ...
          return '<strong>' + esc(short) + '</strong>'
          ...
          + '<div class="sa-body">'
            ...
            + '<div class="sa-actions">'
              + (students.length
                  ...
                  : '')
              + '<button class="sa-btn-ghost" onclick="window.showPage && window.showPage(\'engagement\')">'
                + esc(tr('alert_view_prediction')) + '</button>'
            + '</div>'
          + '</div>'
          + '</div>';
      }
      ...
      function renderSmartAlerts(badgeOnly) {
        ...
        var total = d.sawNotSubmit.length + d.sharpDrop.length + d.ghosts.length
                  + d.symbolic.length + d.nightOwls.length + d.reactivated.length;
        ...
        if (d.sawNotSubmit.length)
          blocks.push(saBlock('sa-danger',  '📬', 'alert_block_viu_title',   'alert_block_viu_tip',   d.sawNotSubmit, null));
    
        if (d.sharpDrop.length)
        ...
        if (d.nightOwls.length)
          blocks.push(saBlock('sa-info',    '🌙', 'alert_block_madru_title', 'alert_block_madru_tip', d.nightOwls,
            function (s) { return trN('alert_extra_madru', s.pct); }));
        ...
        // Delegated handler for message buttons (data-sa-bulk)
        ...
        if (container) {
          container.addEventListener('click', function(e) {
            ...
            if (!btn) return;
            e.stopPropagation();
            // Encontrar o card pai para recuperar o id
