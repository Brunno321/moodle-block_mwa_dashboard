...
define(['block_mwa_dashboard/dashboardstore'], function(Store) {
    ...
    (function () {
      ...
      function renderGrid(filtered, allStudents, activeCompletionBase, logs, emailMap, timeMap) {
        ...
        var html = filtered.map(function (entry, idx) {
          ...
          ACT_DATA[idx] = {
            ...
            duedate: (renderActivities._dueDates && v.cmid && renderActivities._dueDates[v.cmid])
              || (v.availableuntil ? v.availableuntil * 1000 : 0),
            graded: graded
          };
     
          return '<div class="act-card act-card-' + esc(v.availability || 'unknown') + '" onclick="window.MWAActivities.openModal(' + idx + ')">'
      ...
      function openModal(idx) {
        ...
        if (d.graded) {
          if (d.duedate) {
            ...
            var dateStr = dd.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' });
            if (diffDays < 0 || d.availability === 'closed') {
              ...
              dueDateKpi = '<div style="background:var(--panel2);border:1px solid var(--line);border-radius:12px;padding:12px 14px;text-align:center;">'
                  + '<div style="font-size:.65rem;font-weight:900;text-transform:uppercase;letter-spacing:.07em;color:var(--muted);margin-bottom:4px;">' + tr('act_kpi_due', 'Finaliza em') + '</div>'
      ...
      function renderActivities() {
        ...
        var courseid = parseInt((dash.config || {}).courseid || 0, 10);
        ...
        var ACT_TYPES = { forum: 1, tarefa: 1, quiz: 1, h5p: 1 };
        ...
        var filtered = arr.filter(function (e) {
          ...
          if (CURRENT_AVAILABILITY !== 'all' && e[1].availability !== CURRENT_AVAILABILITY) return false;
      ...
      window.MWAActivities = {
        ...
        setFilter: setFilter,
        filterLowEngagement: function() { setFilter('low', document.getElementById('actTabLow')); },
