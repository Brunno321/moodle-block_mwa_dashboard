...
define(['block_mwa_dashboard/dashboardstore'], function(Store) {
    ...
    (function () {
      ...
      var ACT_DATA = [];
      var ACT_PAGE = 1;
      var ACT_PAGE_SIZE = 10;
      var IGNORED_PAGE = 1;
      var IGNORED_PAGE_SIZE = 10;
      var HIDDEN_CMIDS = {};
      var HIDDEN_LOADED = false;
      var HIDDEN_LOADING = false;
      var ACT_EXPORT_ENTRIES = [];
      var ACT_EXPORT_STUDENTS = [];
      var ACT_EXPORT_ACTIVE_BASE = 0;
      ...
      function utf16leBlob(text) {
        ...
        for (var i = 0; i < text.length; i++) view.setUint16(2 + (i * 2), text.charCodeAt(i), true);
        return new Blob([buffer], {type: 'text/csv;charset=utf-16le'});
      }

      function exportActivitiesSpreadsheet() {
        if (!ACT_EXPORT_ENTRIES.length) {
          Store.notify(tr('act_export_empty', 'Não há atividades ou recursos acompanhados para exportar.'), 'warning');
          return;
        }
        var reachBase = Math.max(1, ACT_EXPORT_STUDENTS.length);
        var activeBase = Math.max(1, ACT_EXPORT_ACTIVE_BASE);
        var headers = [
          tr('act_col_activity_resource', 'Atividade/Recurso'), tr('act_export_type', 'Tipo'),
          tr('act_col_completion_rate', 'Taxa de conclusão'), tr('act_col_students_access', 'Alunos com acesso'),
          tr('act_col_access_count', 'Quantidade de acessos'), tr('act_col_access_coverage', 'Cobertura de acesso'),
          tr('act_col_deadline', 'Prazo'), tr('act_export_due_date', 'Data do prazo'),
          tr('act_col_tracking', 'Acompanhamento'), tr('act_export_visibility', 'Visibilidade no painel')
        ];
        var rows = ACT_EXPORT_ENTRIES.map(function(entry) {
          var name = entry[0], item = entry[1];
          var dueDate = (renderActivities._dueDates && item.cmid && renderActivities._dueDates[item.cmid]) ||
            (item.availableuntil ? item.availableuntil * 1000 : 0);
          var availability = cardAvailability(item, dueDate);
          return [
            name, item.type || '', clampPct(item.concluded.size / activeBase * 100) + '%', item.students.size,
            item.count, clampPct(item.students.size / reachBase * 100) + '%', availabilityLabel(availability),
            dueDate ? new Date(dueDate).toLocaleDateString() : '', tr('act_export_tracked', 'Acompanhado'),
            HIDDEN_CMIDS[Number(item.cmid || 0)] ? tr('act_export_hidden', 'Oculto') : tr('act_export_visible', 'Exibido')
          ];
        });
        var csv = 'sep=;\r\n' + [headers].concat(rows).map(function(row) {
          return row.map(csvCell).join(';');
        }).join('\r\n');
        var url = window.URL.createObjectURL(utf16leBlob(csv));
        var link = document.createElement('a');
        var courseId = (Store.getConfig() || {}).courseid || 'curso';
        link.href = url;
        link.download = 'atividades_recursos_' + courseId + '_' + new Date().toISOString().slice(0, 10) + '.csv';
      ...
      var MOD_FORUM = set(['forum', 'glossary', 'data', 'database', 'chat', 'board']);
      /* Submit a file, text, or comparable production. */
      var MOD_TAREFA = set(['assign', 'assignment', 'workshop', 'vpl', 'journal']);
      /* Attempt/check-in based. */
      var MOD_QUIZ = set(['quiz', 'lesson', 'scorm',
        'choice', 'choicegroup', 'groupselect', 'feedback', 'survey', 'questionnaire',
        'game', 'attendance', 'bigbluebuttonbn', 'webconf']);
      /* Consume only. */
      ...
      var TYPE_ICONS  = { forum: '<svg class="mwa-ui-icon"><use href="#mwa-icon-message"></use></svg>', quiz: '<svg class="mwa-ui-icon"><use href="#mwa-icon-circle"></use></svg>', h5p: '<svg class="mwa-ui-icon"><use href="#mwa-icon-puzzle"></use></svg>', tarefa: '<svg class="mwa-ui-icon"><use href="#mwa-icon-edit"></use></svg>', video: '<svg class="mwa-ui-icon"><use href="#mwa-icon-play"></use></svg>', login: '<svg class="mwa-ui-icon"><use href="#mwa-icon-key"></use></svg>', outro: '<svg class="mwa-ui-icon"><use href="#mwa-icon-pin"></use></svg>' };
      ...
      function buildActMap(logs) {
        ...
        Object.values(actMap).forEach(function (v) {
          if (v.type === 'tarefa' || v.type === 'quiz' || v.type === 'h5p') {
      ...
      function mergeCourseModules(actMap, modules, pruneMissing) {
        ...
        (modules || []).forEach(function(module) {
          ...
          if (module.tracked === false) entry.tracked = false;
        });
      }
    
      function renderKPIs(arr, allStudents, activeCompletionBase) {
        var container = document.getElementById('actKpisRow');
        if (!container) return;
        var reachBase = Math.max(1, allStudents.length);
        var deliveryBase = Math.max(1, activeCompletionBase);
        var avgPct   = arr.length
          ? clampPct(arr.reduce(function (s, e) { return s + e[1].students.size; }, 0) / arr.length / reachBase * 100)
          : 0;
        var criticalDelivery = 0;
        var lowCoverage = 0;
        var overdueStudents = {};
        ...
        arr.forEach(function(entry) {
          ...
          var dueDate = (renderActivities._dueDates && item.cmid && renderActivities._dueDates[item.cmid]) ||
            (item.availableuntil ? item.availableuntil * 1000 : 0);
          var availability = cardAvailability(item, dueDate);
          var deliveryPct = clampPct(item.concluded.size / deliveryBase * 100);
          var reachPct = clampPct(item.students.size / reachBase * 100);
          if (availability !== 'future' && hasSubmission(item.type) && deliveryPct < 60) criticalDelivery++;
          if (availability !== 'future' && reachPct < 60) lowCoverage++;
          if (availability === 'closed' && hasSubmission(item.type)) {
            var hasPending = false;
            allStudents.forEach(function(student) {
              if (!item.concluded.has(student)) {
                overdueStudents[student] = true;
                hasPending = true;
              }
            });
            if (hasPending) overdueActivities++;
          }
        });
        var overdueCount = Object.keys(overdueStudents).length;

        Store.renderHtml(container, '<button type="button" class="kpi c-red act-analysis-kpi act-kpi-red" onclick="window.MWAActivities.setFilter(\'deliverycritical\',this)"><div class="act-kpi-head"><span class="act-kpi-icon" aria-hidden="true">' + icon('clipboard') + '</span><div class="kpi-label">' + esc(tr('act_kpi_critical_delivery', 'Entrega crítica')) + tip('act_tip_critical_delivery') + '</div></div><div class="kpi-value">' + criticalDelivery + '</div><div class="kpi-sub">' + esc(tr(criticalDelivery === 1 ? 'act_kpi_critical_delivery_sub_one' : 'act_kpi_critical_delivery_sub', criticalDelivery === 1 ? 'atividade abaixo de 60%' : 'atividades abaixo de 60%')) + '</div></button>'
        + '<button type="button" class="kpi c-amber act-analysis-kpi act-kpi-amber" onclick="window.MWAActivities.setFilter(\'lowcoverage\',this)"><div class="act-kpi-head"><span class="act-kpi-icon" aria-hidden="true">' + icon('eye') + '</span><div class="kpi-label">' + esc(tr('act_kpi_low_coverage', 'Baixa cobertura')) + tip('act_tip_low_coverage') + '</div></div><div class="kpi-value">' + lowCoverage + '</div><div class="kpi-sub">' + esc(tr(lowCoverage === 1 ? 'act_kpi_low_coverage_sub_one' : 'act_kpi_low_coverage_sub', lowCoverage === 1 ? 'item acessado por menos de 60%' : 'itens acessados por menos de 60%')) + '</div></button>'
        + '<button type="button" class="kpi c-red act-analysis-kpi act-kpi-red" onclick="window.MWAActivities.setFilter(\'overdue\',this)"><div class="act-kpi-head"><span class="act-kpi-icon" aria-hidden="true">' + icon('warning') + '</span><div class="kpi-label">' + esc(tr('act_kpi_overdue_pending', 'Pendências vencidas')) + tip('act_tip_overdue_pending') + '</div></div><div class="kpi-value">' + overdueCount + '</div><div class="kpi-sub">' + overdueActivities + ' ' + esc(tr(overdueActivities === 1 ? 'act_kpi_overdue_activity_one' : 'act_kpi_overdue_activities', overdueActivities === 1 ? 'atividade vencida' : 'atividades vencidas')) + '</div></button>'
        + '<div class="kpi c-teal act-analysis-kpi act-kpi-teal"><div class="act-kpi-head"><span class="act-kpi-icon" aria-hidden="true">' + icon('chart') + '</span><div class="kpi-label">' + esc(tr('act_kpi_reach_average', 'Alcance médio')) + tip('act_tip_average_per_resource') + '</div></div><div class="kpi-value">' + avgPct + '%</div><div class="kpi-sub">' + esc(tr('act_kpi_avg_delta')) + '</div></div>');
      }
      ...
      function renderGrid(filtered, allStudents, activeCompletionBase, logs, emailMap, timeMap) {
        ...
        var html = filtered.map(function (entry, idx) {
          ...
          var pct      = clampPct(v.students.size / reachBase * 100);
          var color    = TYPE_COLORS[v.type] || 'var(--muted)';
          var graded   = hasSubmission(v.type);
          var canManageTracking = !!((Store.getConfig ? Store.getConfig() : {}).canmanageactivitytracking);
          var concPct  = clampPct(v.concluded.size / activeBase * 100);
          ...
          /* Visual mapping only: the ring consistently presents the existing
             completion metric; the underlying calculations remain unchanged. */
          var ringPct = concPct;
          var ringColor = deliveryStatus.color;
          var ringMetricLabel = tr('act_col_completion_rate');
          ...
          var concludedLabel = v.type === 'forum'
            ? tr(concludedSt.length === 1 ? 'act_label_posted_single' : 'act_label_posted')
            : (v.type === 'tarefa' || v.type === 'quiz' || v.type === 'h5p')
              ? tr(concludedSt.length === 1 ? 'act_label_submitted_single' : 'act_label_submitted')
              : tr(concludedSt.length === 1 ? 'act_label_accessed_single' : 'act_label_accessed');
          var pendingLabel = v.type === 'forum'
            ? tr(pendingSt.length === 1 ? 'act_label_saw_not_posted_single' : 'act_label_saw_not_posted')
            : (v.type === 'tarefa' || v.type === 'quiz' || v.type === 'h5p')
              ? tr(pendingSt.length === 1 ? 'act_label_saw_not_submitted_single' : 'act_label_saw_not_submitted')
              : tr('act_label_pending');
          ...
          return '<div class="act-card act-card-' + esc(cardState) + '" onclick="window.MWAActivities.openModal(' + idx + ')">'
            + '<div class="act-card-progress" title="' + esc(ringMetricLabel) + ': ' + ringPct + '%" aria-label="' + esc(ringMetricLabel) + ': ' + ringPct + '%" style="--act-progress:' + ringPct + ';--act-progress-color:' + ringColor + ';"><span>' + ringPct + '%</span></div>'
            + '<div class="act-card-info">'
              + '<div class="act-card-name-row"><div class="act-card-name">' + esc(k) + '</div></div><div class="act-card-status-row">'
            ...
            + '<div class="act-card-stats">'
              ...
              + '<div class="act-card-stat"><div class="act-card-stat-label">' + tr('act_col_access_coverage') + '</div><div class="act-card-stat-val" style="color:var(--green);">' + pct + '%</div></div>'
            + '</div>'
            + '<div class="act-card-deadline"><span class="act-status-chip act-schedule-chip ' + esc(cardState) + '"><span>' + esc(scheduleLabel) + '</span></span></div>'
            + '<div class="act-card-tracking">'
              + (canManageTracking && v.cmid ? '<span class="act-card-actions"><button type="button" class="act-hide-btn" title="' + esc(tr('act_hide_item', 'Ocultar')) + '" aria-label="' + esc(tr('act_hide_item', 'Ocultar')) + '" onclick="event.stopPropagation();window.MWAActivities.setHidden(' + Number(v.cmid) + ',true)">' + icon('eye') + '</button><button type="button" class="act-track-btn act-track-icon-btn" title="' + esc(tr('act_stop_tracking')) + '" aria-label="' + esc(tr('act_stop_tracking')) + '" onclick="event.stopPropagation();window.MWAActivities.setTracked(' + Number(v.cmid) + ',false)">' + icon('ban') + '</button></span>' : '&mdash;')
            + '</div>'
      ...
      function renderHidden(hidden, allStudents, activeCompletionBase) {
        var panel = document.getElementById('actHiddenPanel');
        var grid = document.getElementById('actHiddenGrid');
        if (!panel || !grid) return;
        panel.hidden = !hidden.length;
        if (!hidden.length) {
          Store.renderHtml(grid, '');
          return;
        }
        var pendingCount = 0;
        var html = hidden.map(function(entry) {
          var item = entry[1];
          var dueDate = (renderActivities._dueDates && item.cmid && renderActivities._dueDates[item.cmid]) ||
            (item.availableuntil ? item.availableuntil * 1000 : 0);
          var state = cardAvailability(item, dueDate);
          var pending = hasSubmission(item.type) && state === 'closed' &&
            item.concluded.size < Math.max(1, activeCompletionBase);
          if (pending) pendingCount++;
          return '<div class="act-hidden-item"><span class="act-hidden-item-name">' + esc(entry[0]) + '</span>'
            + (pending ? '<span class="act-hidden-warning">' + icon('warning') + ' ' + esc(tr('act_hidden_pending_one', 'Com pendência')) + '</span>' : '')
            + '<button type="button" class="act-hide-btn is-show" onclick="window.MWAActivities.setHidden(' + Number(item.cmid) + ',false)">' + icon('eye') + ' ' + esc(tr('act_show_item', 'Mostrar novamente')) + '</button></div>';
        }).join('');
        Store.renderHtml(grid, html);
        var count = document.getElementById('actHiddenCount');
        var pending = document.getElementById('actHiddenPending');
        if (count) count.textContent = hidden.length;
        if (pending) pending.textContent = pendingCount ? tr('act_hidden_pending', '{count} com pendências')
          .replace('{$a}', pendingCount).replace('{count}', pendingCount) : '';
      }

      function setHidden(cmid, hidden) {
        var config = Store.getConfig ? Store.getConfig() : {};
        Store.callAction('block_mwa_dashboard_set_activity_hidden', {
          courseid: Number(config.courseid || 0), cmid: Number(cmid || 0), hidden: !!hidden
        }).then(function(response) {
          HIDDEN_CMIDS = {};
          ((response && response.cmids) || []).forEach(function(id) { HIDDEN_CMIDS[Number(id)] = true; });
          HIDDEN_LOADED = true;
          ACT_PAGE = 1;
          renderActivities();
        }).catch(function() {
          Store.notify(tr('act_hidden_error', 'Não foi possível alterar a visibilidade do item.'), 'error');
        });
      }

      function renderIgnored(ignored) {
        var panel = document.getElementById('actIgnoredPanel');
        var grid = document.getElementById('actIgnoredGrid');
        if (!panel || !grid) return;
        panel.hidden = !ignored.length;
        var count = document.getElementById('actIgnoredCount');
        if (count) count.textContent = ignored.length;
        if (!ignored.length) {
          Store.renderHtml(grid, '');
          renderIgnoredPagination(0);
          return;
        }
        var totalPages = Math.max(1, Math.ceil(ignored.length / IGNORED_PAGE_SIZE));
        IGNORED_PAGE = Math.min(Math.max(1, IGNORED_PAGE), totalPages);
        var start = (IGNORED_PAGE - 1) * IGNORED_PAGE_SIZE;
        var pageItems = ignored.slice(start, start + IGNORED_PAGE_SIZE);
        var canManage = !!((Store.getConfig ? Store.getConfig() : {}).canmanageactivitytracking);
        var html = pageItems.map(function(entry) {
            var item = entry[1];
            return '<div class="act-ignored-item"><span>' + esc(entry[0]) + '</span>'
              + (canManage && item.cmid ? '<button type="button" class="act-track-btn is-resume" onclick="window.MWAActivities.setTracked(' + Number(item.cmid) + ',true)">&#8635; ' + esc(tr('act_resume_tracking')) + '</button>' : '')
              + '</div>';
          }).join('');
        Store.renderHtml(grid, html);
        renderIgnoredPagination(ignored.length);
      }

      function setTracked(cmid, tracked) {
        var config = Store.getConfig ? Store.getConfig() : {};
        if (!tracked && !window.confirm(tr('act_tracking_confirm'))) return;
        if (!config.courseid) {
          Store.notify(tr('act_tracking_error'), 'error');
          return;
        }
        Store.callAction('block_mwa_dashboard_set_activity_tracking', {
          courseid: Number(config.courseid || 0),
          cmid: Number(cmid || 0),
          tracked: !!tracked
        }).then(function() {
          (config.activitylinks || []).forEach(function(item) {
            if (Number(item && item.cmid || 0) === Number(cmid || 0)) item.tracked = !!tracked;
          });
          ...
          renderActivities();
          if (typeof window.MWAReloadData === 'function') {
            return window.MWAReloadData(true).then(function() {
              // receiveData rebuilds every dependent module; render this tab once more
              // so the tracking controls always reflect the freshly loaded state.
              renderActivities();
            });
          }
          return null;
      ...
      function renderAvailabilityLegend() {
        ...
        Store.renderHtml(box,
          ...
          + '<span class="act-legend-separator" aria-hidden="true"></span>'
          + '<span class="act-legend-item act-action-legend"><i class="act-legend-action hide">' + icon('eye') + '</i>' + esc(tr('act_hide_item', 'Ocultar')) + '</span>'
          + '<span class="act-legend-item act-action-legend"><i class="act-legend-action untrack">' + icon('ban') + '</i>' + esc(tr('act_stop_tracking', 'Não acompanhar')) + '</span>'
        );
      }

      function renderPagination(totalItems) {
        var box = document.getElementById('actPagination');
        if (!box) return;
        var totalPages = Math.max(1, Math.ceil(totalItems / ACT_PAGE_SIZE));
        ACT_PAGE = Math.min(Math.max(1, ACT_PAGE), totalPages);
        var first = totalItems ? ((ACT_PAGE - 1) * ACT_PAGE_SIZE) + 1 : 0;
        var last = Math.min(ACT_PAGE * ACT_PAGE_SIZE, totalItems);
        var buttons = '<button type="button" class="act-pag-btn"' + (ACT_PAGE === 1 ? ' disabled' : '') + ' onclick="window.MWAActivities.setPage(' + (ACT_PAGE - 1) + ')">&#8249;</button>';
        for (var page = 1; page <= totalPages; page++) {
          buttons += '<button type="button" class="act-pag-btn' + (page === ACT_PAGE ? ' active' : '') + '" onclick="window.MWAActivities.setPage(' + page + ')">' + page + '</button>';
        }
        buttons += '<button type="button" class="act-pag-btn"' + (ACT_PAGE === totalPages ? ' disabled' : '') + ' onclick="window.MWAActivities.setPage(' + (ACT_PAGE + 1) + ')">&#8250;</button>';
        var sizes = [5, 10, 20, 50].map(function(size) {
          return '<option value="' + size + '"' + (size === ACT_PAGE_SIZE ? ' selected' : '') + '>' + size + '</option>';
        }).join('');
        Store.renderHtml(box, '<div class="act-pag-info">' + esc(tr('act_pag_showing').replace('{start}', first).replace('{end}', last).replace('{total}', totalItems)) + '</div>'
          + '<div class="act-pag-pages">' + buttons + '</div>'
          + '<label class="act-pag-size">' + esc(tr('act_items_per_page')) + '<select onchange="window.MWAActivities.setPageSize(this.value)">' + sizes + '</select></label>'
        );
      }

      function renderIgnoredPagination(totalItems) {
        var box = document.getElementById('actIgnoredPagination');
        if (!box) return;
        var totalPages = Math.max(1, Math.ceil(totalItems / IGNORED_PAGE_SIZE));
        IGNORED_PAGE = Math.min(Math.max(1, IGNORED_PAGE), totalPages);
        var first = totalItems ? ((IGNORED_PAGE - 1) * IGNORED_PAGE_SIZE) + 1 : 0;
        var last = Math.min(IGNORED_PAGE * IGNORED_PAGE_SIZE, totalItems);
        var buttons = '<button type="button" class="act-pag-btn"' + (IGNORED_PAGE === 1 ? ' disabled' : '') + ' onclick="window.MWAActivities.setIgnoredPage(' + (IGNORED_PAGE - 1) + ')">&#8249;</button>';
        for (var page = 1; page <= totalPages; page++) {
          buttons += '<button type="button" class="act-pag-btn' + (page === IGNORED_PAGE ? ' active' : '') + '" onclick="window.MWAActivities.setIgnoredPage(' + page + ')">' + page + '</button>';
        }
        buttons += '<button type="button" class="act-pag-btn"' + (IGNORED_PAGE === totalPages ? ' disabled' : '') + ' onclick="window.MWAActivities.setIgnoredPage(' + (IGNORED_PAGE + 1) + ')">&#8250;</button>';
        var sizes = [5, 10, 20, 50].map(function(size) {
          return '<option value="' + size + '"' + (size === IGNORED_PAGE_SIZE ? ' selected' : '') + '>' + size + '</option>';
        }).join('');
        Store.renderHtml(box,
          '<div class="act-pag-info">' + esc(tr('act_pag_showing').replace('{start}', first).replace('{end}', last).replace('{total}', totalItems)) + '</div>'
          + '<div class="act-pag-pages">' + buttons + '</div>'
          + '<label class="act-pag-size">' + esc(tr('act_items_per_page')) + '<select onchange="window.MWAActivities.setIgnoredPageSize(this.value)">' + sizes + '</select></label>'
        );
      }

      function setPage(page) {
        ACT_PAGE = Math.max(1, Number(page) || 1);
        renderActivities();
      }

      function setPageSize(size) {
        ACT_PAGE_SIZE = Math.max(1, Number(size) || 10);
        ACT_PAGE = 1;
        renderActivities();
      }

      function setIgnoredPage(page) {
        IGNORED_PAGE = Math.max(1, Number(page) || 1);
        renderActivities();
      }

      function setIgnoredPageSize(size) {
        IGNORED_PAGE_SIZE = Math.max(1, Number(size) || 10);
        IGNORED_PAGE = 1;
        renderActivities();
      }

      function openNameSection(section) {
        var overlay = document.getElementById('actModalOverlay');
        if (!overlay) return;
        var details = overlay.querySelector('.act-name-section.' + section);
        if (!details) return;
        var shouldOpen = !details.open;
        overlay.querySelectorAll('.act-name-section[open]').forEach(function(item) { item.open = false; });
      ...
      function openModal(idx) {
        ...
        var dueDateKpi = '';
        if (d.graded) {
          var deadlineTitle = esc(tr('act_col_deadline', 'Prazo'));
          if (d.duedate) {
            ...
            if (d.availability === 'closed') {
              dueDateKpi = '<div class="act-detail-box act-deadline-box">'
                  + '<div class="act-detail-label">' + deadlineTitle + '</div>'
                  + '<div class="act-deadline-content"><div class="act-deadline-state" style="color:var(--red);">' + esc(tr('act_kpi_closed', 'Finalizada')) + '</div>'
                  + '<div class="act-deadline-date">' + esc(dateStr) + '</div></div>'
              + '</div>';
            } else {
              var dateColor = diffDays <= 3 ? 'var(--amber, #f59e0b)' : 'var(--blue)';
              var deadlineState = d.availability === 'today'
                ? tr('act_availability_today', 'Finaliza hoje')
                : tr('act_availability_open', 'Aberta');
              dueDateKpi = '<div class="act-detail-box act-deadline-box">'
                  + '<div class="act-detail-label">' + deadlineTitle + '</div>'
                  + '<div class="act-deadline-content"><div class="act-deadline-state" style="color:' + dateColor + ';">' + esc(deadlineState) + '</div>'
                  + '<div class="act-deadline-date">' + esc(tr('act_kpi_due', 'Finaliza em')) + ' ' + esc(dateStr) + '</div></div>'
              + '</div>';
            }
          } else if (d.availability === 'closed') {
            dueDateKpi = '<div class="act-detail-box act-deadline-box">'
                + '<div class="act-detail-label">' + deadlineTitle + '</div>'
                + '<div class="act-deadline-content"><div class="act-deadline-state" style="color:var(--red);">' + esc(tr('act_kpi_closed', 'Finalizada')) + '</div></div>'
            + '</div>';
          } else {
            dueDateKpi = '<div class="act-detail-box act-deadline-box">'
                + '<div class="act-detail-label">' + deadlineTitle + '</div>'
                + '<div class="act-deadline-content"><div class="act-deadline-state" style="color:var(--green);">' + esc(tr('act_kpi_no_deadline', 'Sem prazo')) + '</div></div>'
            + '</div>';
          }
        }

        var kpiHtml =
          '<div class="act-modal-kpi-grid' + (d.graded ? '' : ' act-modal-kpi-grid-two') + '">'
          + (d.graded
            ? '<button type="button" class="act-modal-kpi act-modal-kpi-link" onclick="window.MWAActivities.openNameSection(\'done\')">'
                + '<div class="act-detail-label">' + esc(d.concludedLabel) + '</div>'
                + '<div style="font-size:1.6rem;font-weight:900;color:var(--green);">' + d.concludedSt.length + '</div>'
              + '</button>'
              + '<button type="button" class="act-modal-kpi act-modal-kpi-link" onclick="window.MWAActivities.openNameSection(\'pending\')">'
                + '<div class="act-detail-label">' + esc(d.pendingLabel) + '</div>'
            ...
            : '<button type="button" class="act-modal-kpi act-modal-kpi-link" onclick="window.MWAActivities.openNameSection(\'done\')">'
                ...
                + '<div style="font-size:1.6rem;font-weight:900;color:var(--green);">' + d.concludedSt.length + '</div>'
              + '</button>')
          + '<button type="button" class="act-modal-kpi act-modal-kpi-link" onclick="window.MWAActivities.openNameSection(\'none\')">'
              + '<div class="act-detail-label">' + esc(tr('act_modal_no_access')) + '</div>'
              + '<div style="font-size:1.6rem;font-weight:900;color:' + (d.notAccessedSt.length > 0 ? 'var(--red)' : 'var(--green)') + ';">' + d.notAccessedSt.length + '</div>'
          + '</button>'
          + '</div>';
    
        function metricProgressHtml(pct, status) {
          return '<div class="act-metric-progress" style="--act-metric-color:' + status.color + ';">'
            + '<div class="act-metric-track">'
              + '<div class="act-metric-fill" style="width:' + pct + '%;"></div>'
            + '</div>'
            + '<strong>' + pct + '%</strong>'
          + '</div>';
        }
        var progressHtml = metricProgressHtml(d.pct, d.coverageStatus);
        var completionProgressHtml = metricProgressHtml(d.concPct, d.deliveryStatus);

        /* Resources have no submission, so only the coverage box applies. */
        var statusHtml = '<div class="act-detail-grid' + (d.graded ? ' act-detail-grid-three' : ' single') + '">'
          + (d.graded
            ? '<div class="act-detail-box">'
                + '<div class="act-detail-label">' + esc(tr('act_modal_completion_rate')) + '</div>'
                + detailStatus(d.deliveryStatus)
                + completionProgressHtml
        ...
        if (d.timeRanking.length) {
          timeHtml = '<div class="act-time-panel act-time-collapsed">'
            + '<button type="button" class="act-time-toggle" id="actToggleTime" aria-expanded="false">'
              + '<span class="act-time-title"><svg class="mwa-ui-icon" aria-hidden="true"><use href="#mwa-icon-clock"></use></svg><span>' + tr('act_modal_time_per_student') + '</span></span>'
        ...
        var footerHtml = '<div style="display:flex;justify-content:flex-start;gap:8px;padding:4px 20px 16px;flex-wrap:wrap;">'
          + (d.pendingSt.length
              ? '<button class="act-msg-btn" id="actMsgPend">'
                + '<svg class="mwa-ui-icon" aria-hidden="true"><use href="#mwa-icon-mail"></use></svg>' + tr('act_msg_pending') + ' (' + d.pendingSt.length + ')'
                + '</button>'
              : '')
          + (d.notAccessedSt.length
              ? '<button class="act-msg-btn" id="actMsgNoAcc">'
                + '<svg class="mwa-ui-icon" aria-hidden="true"><use href="#mwa-icon-mail"></use></svg>' + tr('act_msg_no_access') + ' (' + d.notAccessedSt.length + ')'
                + '</button>'
              : '')
          + '<button style="background:var(--panel);border:1px solid var(--line2);border-radius:12px;padding:9px 18px;font-family:inherit;font-size:.8rem;font-weight:800;color:var(--muted);cursor:pointer;margin-left:auto;" '
      ...
      function renderActivities() {
        ...
        if (courseid && !HIDDEN_LOADED && !HIDDEN_LOADING) {
          HIDDEN_LOADING = true;
          Store.callAction('block_mwa_dashboard_get_hidden_activities', {courseid: courseid}).then(function(response) {
            HIDDEN_CMIDS = {};
            ((response && response.cmids) || []).forEach(function(id) { HIDDEN_CMIDS[Number(id)] = true; });
            HIDDEN_LOADED = true;
        ...
        if (courseid && !renderActivities._dueDatesFetched) {
          ...
          try {
            Store.callAction('block_mwa_dashboard_get_due_dates', { courseid: courseid }).then(function(res) {
              ...
              renderActivities();
              if (window.MWAActionCenter && typeof window.MWAActionCenter.render === 'function') {
                window.MWAActionCenter.render();
              }
            }).catch(function() {});
          } catch(e) {}
        }
    
        var allEntries = Object.entries(actMap).sort(function (a, b) { return b[1].count - a[1].count; });
        var tracked = allEntries.filter(function(entry) { return entry[1].tracked !== false; });
        var hidden = tracked.filter(function(entry) { return !!HIDDEN_CMIDS[Number(entry[1].cmid || 0)]; });
        var arr = tracked.filter(function(entry) { return !HIDDEN_CMIDS[Number(entry[1].cmid || 0)]; });
        var ignored = allEntries.filter(function(entry) { return entry[1].tracked === false; });
        var seen = {};
        var allStudents = [];
        logs.forEach(function (r) {
          var n = studentName(r);
        ...
        ACT_EXPORT_ENTRIES = tracked.slice();
        ACT_EXPORT_STUDENTS = allStudents.slice();
        ACT_EXPORT_ACTIVE_BASE = activeCompletionBase;
        ...
        var filtered = arr.filter(function (e) {
          ...
          } else if (CURRENT_FILTER === 'low' || CURRENT_FILTER === 'deliverycritical' || CURRENT_FILTER === 'lowcoverage' || CURRENT_FILTER === 'overdue') {
            var rp = clampPct(e[1].students.size / lowReachBase * 100);
            var dp = clampPct(e[1].concluded.size / lowActiveBase * 100);
            var supportsDelivery = hasSubmission(e[1].type);
            if (CURRENT_FILTER === 'deliverycritical') {
              okType = filterAvailability !== 'future' && supportsDelivery && dp < 60;
            } else if (CURRENT_FILTER === 'lowcoverage') {
              okType = filterAvailability !== 'future' && rp < 60;
            } else if (CURRENT_FILTER === 'overdue') {
              okType = filterAvailability === 'closed' && supportsDelivery && e[1].concluded.size < lowActiveBase;
            } else {
              okType = filterAvailability !== 'future' && filterAvailability !== 'closed' &&
                (rp < 60 || (supportsDelivery && dp < 60));
            }
          } else {
        ...
        renderKPIs(tracked, allStudents, activeCompletionBase);
        var totalPages = Math.max(1, Math.ceil(filtered.length / ACT_PAGE_SIZE));
        ACT_PAGE = Math.min(ACT_PAGE, totalPages);
        var pageStart = (ACT_PAGE - 1) * ACT_PAGE_SIZE;
        renderGrid(filtered.slice(pageStart, pageStart + ACT_PAGE_SIZE), allStudents, activeCompletionBase, logs, emailMap, timeMap);
        renderPagination(filtered.length);
        renderHidden(hidden, allStudents, activeCompletionBase);
      ...
      function getLowPerformanceItems() {
        ...
        return Object.entries(actMap).filter(function(entry) {
          return entry[1].tracked !== false;
        }).map(function(entry) {
          var name = entry[0];
          var item = entry[1];
          var dueDate = (renderActivities._dueDates && item.cmid && renderActivities._dueDates[item.cmid]) ||
            (item.availableuntil ? item.availableuntil * 1000 : 0);
          var availability = cardAvailability(item, dueDate);
          if (availability === 'future' || availability === 'closed') return null;
          var reachPct = clampPct(item.students.size / reachBase * 100);
          var supportsDelivery = hasSubmission(item.type);
          var deliveryPct = supportsDelivery ? clampPct(item.concluded.size / deliveryBase * 100) : null;
          var lowReach = reachPct < 60;
          var lowDelivery = supportsDelivery && deliveryPct < 60;
          if (!lowReach && !lowDelivery) return null;
          var worst = lowDelivery ? Math.min(reachPct, deliveryPct) : reachPct;
          return {
            name: name,
            pct: reachPct,
            deliveryPct: deliveryPct,
            lowReach: lowReach,
            lowDelivery: lowDelivery,
            worst: worst,
            unique: item.students.size,
            value: worst + '%',
            cmid: item.cmid || 0,
      ...
      function getChatSnapshot() {
        ...
        return Object.entries(actMap).filter(function(entry) {
          ...
          return {
            ...
            available: v.availability === 'closed' || v.availability === 'future' ? 0 : 1,
            availableuntil: Number(v.availableuntil || 0),
            duedate: Number(v.duedate || 0),
            totalConsiderado: denominator,
            cobertura: denominator ? clampPct(unique / denominator * 100) : 0,
      ...
      window.MWAActivities = {
        ...
        setIgnoredPage: setIgnoredPage,
        setIgnoredPageSize: setIgnoredPageSize,
        openNameSection: openNameSection,
        getChatSnapshot: getChatSnapshot,
        getLowPerformanceItems: getLowPerformanceItems,
        exportSpreadsheet: exportActivitiesSpreadsheet
      };
