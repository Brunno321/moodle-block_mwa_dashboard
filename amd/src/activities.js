...
define(['block_mwa_dashboard/dashboardstore'], function(Store) {
    ...
    (function () {
      ...
      function renderIgnored(ignored, replace) {
        var grid = document.getElementById('actResourceGrid');
        if (!grid || !ignored.length) return;
        var canManage = !!((Store.getConfig ? Store.getConfig() : {}).canmanageactivitytracking);
        var html = '<section class="act-ignored"><div class="act-ignored-head"><i class="act-ignored-icon" aria-hidden="true"><svg class="mwa-ui-icon"><use href="#mwa-icon-ban"></use></svg></i><strong>' + esc(tr('act_ignored_title')) + '</strong><span>' + esc(tr('act_ignored_help')) + '</span></div>'
          + ignored.map(function(entry) {
            var item = entry[1];
        ...
        if (replace) {
          ...
          Store.renderHtml(fragmentHost, html);
          while (fragmentHost.firstChild) grid.appendChild(fragmentHost.firstChild);
        }
      }
      ...
      function setTracked(cmid, tracked) {
        ...
        if (!config.activitytrackingurl) {
          Store.notify(tr('act_tracking_error'), 'error');
          return;
        }
        var separator = config.activitytrackingurl.indexOf('?') >= 0 ? '&' : '?';
        window.location.replace(config.activitytrackingurl + separator + 'cmid=' + encodeURIComponent(Number(cmid || 0))
          + '&tracked=' + (tracked ? '1' : '0'));
      }
      ...
      function renderActivities() {
        ...
        if (CURRENT_FILTER === 'ignored') {
          renderKPIs(arr, allStudents, ignored.length);
          if (ignored.length) {
            var ignoredPages = Math.max(1, Math.ceil(ignored.length / ACT_PAGE_SIZE));
            ACT_PAGE = Math.min(ACT_PAGE, ignoredPages);
            var ignoredStart = (ACT_PAGE - 1) * ACT_PAGE_SIZE;
            renderIgnored(ignored.slice(ignoredStart, ignoredStart + ACT_PAGE_SIZE), true);
          } else {
            renderGrid([], allStudents, activeCompletionBase, logs, emailMap, timeMap);
          }
          renderPagination(ignored.length);
          return;
        }
        var filtered = arr.filter(function (e) {
          ...
          if (CURRENT_FILTER === 'all') {
            ...
            var supportsDelivery = !!(e[1].cmid && gradedCmids[String(e[1].cmid)]) ||
              !!gradedNames[String(e[0] || '').toLowerCase()];
        ...
        if (CURRENT_FILTER === 'all') renderIgnored(ignored, false);
