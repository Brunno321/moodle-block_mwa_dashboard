...
define(['block_mwa_dashboard/dashboardstore'], function(Store) {
    ...
    (function () {
        ...
        var html = filtered.map(function (entry, idx) {
          ...
          return '<div class="act-card act-card-' + esc(cardState) + '" onclick="window.MWAActivities.openModal(' + idx + ')">'
            + '<div class="act-card-title-row">'
              ...
              + (canManageTracking && v.cmid ? '<button type="button" class="act-track-btn" title="' + esc(tr('act_stop_tracking')) + '" onclick="event.stopPropagation();window.MWAActivities.setTracked(' + Number(v.cmid) + ',false)">&#8856; ' + esc(tr('act_stop_tracking')) + '</button>' : '')
            + '</div>'
      ...
      function renderIgnored(ignored, replace) {
        ...
        var html = '<section class="act-ignored"><div class="act-ignored-head"><strong>' + esc(tr('act_ignored_title')) + '</strong><span>' + esc(tr('act_ignored_help')) + '</span></div>'
        ...
        if (d.timeRanking.length) {
          timeHtml = '<div class="act-time-panel act-time-collapsed">'
            + '<button type="button" class="act-time-toggle" id="actToggleTime" aria-expanded="false">'
              + '<span style="font-size:.78rem;font-weight:900;color:var(--text);"><svg class="mwa-ui-icon" aria-hidden="true"><use href="#mwa-icon-clock"></use></svg> ' + tr('act_modal_time_per_student') + '</span>'
