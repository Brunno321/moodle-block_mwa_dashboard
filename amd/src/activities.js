    ...
    (function () {
      ...
      function renderKPIs(arr, allStudents, filteredCount) {
        ...
        var avgPct   = arr.length
          ? clampPct(arr.reduce(function (s, e) { return s + e[1].students.size; }, 0) / arr.length / Math.max(1, allStudents.length) * 100)
          : 0;
    
        Store.renderHtml(container, '<div class="kpi c-blue act-analysis-kpi act-kpi-blue"><div class="act-kpi-head"><span class="act-kpi-icon" aria-hidden="true"><svg class="mwa-ui-icon"><use href="#mwa-icon-clipboard"></use></svg></span><div class="kpi-label">' + tr('act_kpi_unique') + tip('act_tip_unique_resources') + '</div></div><div class="kpi-value">' + arr.length + '</div><div class="kpi-sub">' + filteredCount + ' ' + tr('act_kpi_unique_shown') + '</div></div>'
        + '<div class="kpi c-green act-analysis-kpi act-kpi-green"><div class="act-kpi-head"><span class="act-kpi-icon" aria-hidden="true"><svg class="mwa-ui-icon"><use href="#mwa-icon-eye"></use></svg></span><div class="kpi-label">' + tr('act_kpi_total_acc') + tip('act_tip_total_accesses') + '</div></div><div class="kpi-value">' + totalAcc.toLocaleString() + '</div><div class="kpi-sub" aria-hidden="true">&nbsp;</div></div>'
        + '<div class="kpi c-amber act-analysis-kpi act-kpi-amber"><div class="act-kpi-head"><span class="act-kpi-icon" aria-hidden="true"><svg class="mwa-ui-icon"><use href="#mwa-icon-users"></use></svg></span><div class="kpi-label">' + tr('act_kpi_students') + tip('act_tip_distinct_students') + '</div></div><div class="kpi-value">' + allStudents.length + '</div><div class="kpi-sub" aria-hidden="true">&nbsp;</div></div>'
        + '<div class="kpi c-teal act-analysis-kpi act-kpi-teal"><div class="act-kpi-head"><span class="act-kpi-icon" aria-hidden="true"><svg class="mwa-ui-icon"><use href="#mwa-icon-chart"></use></svg></span><div class="kpi-label">' + tr('act_kpi_avg') + tip('act_tip_average_per_resource') + '</div></div><div class="kpi-value">' + avgPct + '%</div><div class="kpi-sub">' + tr('act_kpi_avg_delta') + '</div></div>');
      }
        ...
        var html = filtered.map(function (entry, idx) {
          ...
          return '<div class="act-card act-card-' + esc(cardState) + '" onclick="window.MWAActivities.openModal(' + idx + ')">'
            ...
            + '<div class="act-card-tracking">'
              + (canManageTracking && v.cmid ? '<button type="button" class="act-track-btn" title="' + esc(tr('act_stop_tracking')) + '" onclick="event.stopPropagation();window.MWAActivities.setTracked(' + Number(v.cmid) + ',false)">&#8856; ' + esc(tr('act_stop_tracking')) + '</button>' : '&mdash;')
            + '</div>'
      ...
      function openModal(idx) {
        ...
        if (d.graded) {
          if (d.duedate) {
            ...
            if (d.availability === 'closed') {
              dueDateKpi = '<div class="act-modal-kpi act-modal-kpi-static">'
        ...
        var kpiHtml =
          '<div class="act-modal-kpi-grid" style="--act-modal-kpi-cols:' + kpiCols + ';">'
          + (d.graded
            ? '<button type="button" class="act-modal-kpi act-modal-kpi-link" onclick="window.MWAActivities.openNameSection(\'done\')">'
                + '<div style="font-size:.65rem;font-weight:900;text-transform:uppercase;letter-spacing:.07em;color:var(--muted);margin-bottom:4px;">' + esc(d.concludedLabel) + '</div>'
                + '<div style="font-size:1.6rem;font-weight:900;color:var(--green);">' + d.concludedSt.length + '</div>'
                + '<div class="act-modal-kpi-sub">' + esc(d.concludedLabel) + '</div>'
              + '</button>'
              + '<button type="button" class="act-modal-kpi act-modal-kpi-link" onclick="window.MWAActivities.openNameSection(\'pending\')">'
                + '<div style="font-size:.65rem;font-weight:900;text-transform:uppercase;letter-spacing:.07em;color:var(--muted);margin-bottom:4px;">' + tr('ct_not_delivered') + '</div>'
                + '<div style="font-size:1.6rem;font-weight:900;color:' + (d.pendingSt.length > 0 ? 'var(--red)' : 'var(--green)') + ';">' + d.pendingSt.length + '</div>'
                + '<div class="act-modal-kpi-sub">' + esc(d.pendingLabel) + '</div>'
              + '</button>'
            : '<button type="button" class="act-modal-kpi act-modal-kpi-link" onclick="window.MWAActivities.openNameSection(\'done\')">'
                + '<div style="font-size:.65rem;font-weight:900;text-transform:uppercase;letter-spacing:.07em;color:var(--muted);margin-bottom:4px;">' + esc(d.concludedLabel) + '</div>'
                + '<div style="font-size:1.6rem;font-weight:900;color:var(--green);">' + d.concludedSt.length + '</div>'
          ...
          + '<button type="button" class="act-modal-kpi act-modal-kpi-link" onclick="window.MWAActivities.openNameSection(\'none\')">'
              + '<div style="font-size:.65rem;font-weight:900;text-transform:uppercase;letter-spacing:.07em;color:var(--muted);margin-bottom:4px;">' + tr('act_modal_no_access') + '</div>'
        ...
        function metricProgressHtml(pct, status) {
          return '<div class="act-metric-progress" style="--act-metric-color:' + status.color + ';">'
            + '<div class="act-metric-track">'
              + '<div class="act-metric-fill" style="width:' + pct + '%;min-width:' + (pct > 0 ? '44px' : '0') + ';">'
                + (pct > 0 ? '<span>' + pct + '%</span>' : '')
              + '</div>'
              + (pct === 0 ? '<span class="act-metric-zero">0%</span>' : '')
            + '</div>'
          + '</div>';
        }
        ...
        var statusHtml = '<div class="act-detail-grid' + (d.graded ? '' : ' single') + '">'
          + (d.graded
        ...
        var footerHtml = '<div style="display:flex;justify-content:flex-start;gap:8px;padding:14px 20px 16px;border-top:1px solid var(--line);flex-wrap:wrap;">'
          + (d.pendingSt.length
      ...
      function renderActivities() {
        ...
        var arr = allEntries.filter(function(entry) { return entry[1].tracked !== false; });
