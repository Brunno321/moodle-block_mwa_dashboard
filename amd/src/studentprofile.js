...
define(['block_mwa_dashboard/dashboardstore', 'block_mwa_dashboard/engagementcalc', 'core/templates'], function(Store, EngagementCalc, Templates) {
    ...
    (function () {
      ...
      function engLabel(score) {
        if (score === 0) return { label: tr('ev_never_access', 'Nunca acessou o Moodle'), color: 'var(--red)' };
      ...
      function buildActivityProgress(name, grades) {
        ...
        return '<div class="cl-act-dots">' + seqs.map(function (seq) {
          ...
          var title = esc(actName) + (done ? ' (' + esc(tr('cl_delivered_single')) + ')' : ' (-)');
          if (url) return '<a class="cl-act-dot ' + cls + '" href="' + esc(url) + '" target="_blank" rel="noopener" title="' + title + '">' + esc(seq) + '</a>';
      ...
      function buildResourceCatalog(logs) {
        ...
        (logs || []).forEach(function (log) {
          ...
          var resName = !isCourseGeneral(ctx) ? ctx : (!isCourseGeneral(ev) ? ev : '');
          if (!resName) return;
          resName = resName.replace(/^(page|página|book|livro|url|resource|recurso|file|arquivo|folder|pasta)\s*:\s*/i, '');
          var cmid = parseInt(log.cmid || log._cmid || log.contextinstanceid || log.contextinstance || log.coursemoduleid || log.moduleid || 0, 10) || 0;
          var mod = moduleOf(log);
          var key = cmid ? 'cmid:' + cmid : 'name:' + resName.toLowerCase();
          if (!map[key]) map[key] = {name: resName, cmid: cmid, mod: mod, url: norm(log.url || log.contexturl || log.objecturl || log.link || log.viewurl || log._url || ''), students: {}};
      ...
      function buildResourceProgress(name, logs) {
        ...
        return '<div class="cl-act-dots">' + resources.map(function (res, idx) {
          ...
          if (url) return '<a class="cl-act-dot ' + cls + '" href="' + esc(url) + '" target="_blank" rel="noopener" title="' + title + '">' + label + '</a>';
      ...
      function populateSelect(preferredName) {
        /* kept for compatibility with external modules that call MWAProfile.render()
           or dispatch spStudentSel change events; syncs the hidden select + autocomplete input */
        var name = norm(preferredName) || norm(PENDING_PROFILE_STUDENT);
        if (!name) return;
        var input = document.getElementById('spStudentSearch');
        var clear = document.getElementById('spSearchClear');
        var sel   = document.getElementById('spStudentSel');
        if (input && input.value.toLowerCase() !== name.toLowerCase()) {
          input.value = name;
      ...
      function loadProfile(name) {
        ...
        var resourceProgressHtml = buildResourceProgress(name, logs);
      ...
      /* autocomplete */
      function acNameNode(name, term) {
        var node = document.createElement('span');
        node.className = 'sp-ac-name';
        var idx = term ? name.toLowerCase().indexOf(term.toLowerCase()) : -1;
        if (idx < 0) {
          node.textContent = name;
          return node;
        }
        node.appendChild(document.createTextNode(name.slice(0, idx)));
        var mark = document.createElement('span');
        mark.className = 'sp-ac-match';
        mark.textContent = name.slice(idx, idx + term.length);
        node.appendChild(mark);
        node.appendChild(document.createTextNode(name.slice(idx + term.length)));
        return node;
      }

      function render() {
        var input  = document.getElementById('spStudentSearch');
        var drop   = document.getElementById('spAutocompleteDrop');
        var clear  = document.getElementById('spSearchClear');
        var wrap   = document.getElementById('spAutocompleteWrap');
        if (!input || input.dataset.spInit) return;
        input.dataset.spInit = '1';
        ...
        function openDrop(names, term) {
          AC_NAMES = names;
          AC_ACTIVE = -1;
          drop.replaceChildren();
          if (!names.length) {
            var empty = document.createElement('li');
            empty.className = 'sp-ac-empty';
            empty.textContent = tr('sp_search_no_results');
            drop.appendChild(empty);
          } else {
            names.slice(0, 40).forEach(function(n, i) {
              var li = document.createElement('li');
              li.setAttribute('role', 'option');
              li.dataset.idx = i;
              li.dataset.name = n;
              li.appendChild(acNameNode(n, term));
        ...
        function selectName(name) {
          ...
          closeDrop();
          loadProfile(name);
          /* keep hidden select in sync for goToStudentProfile compat */
          var sel = document.getElementById('spStudentSel');
          if (sel) { sel.value = name; }
        }

        function setActive(idx) {
          var items = drop.querySelectorAll('li[data-idx]');
          AC_ACTIVE = Math.max(-1, Math.min(idx, items.length - 1));
          Array.prototype.forEach.call(items, function(li, i) {
            li.classList.toggle('sp-ac-active', i === AC_ACTIVE);
          });
          if (AC_ACTIVE >= 0 && items[AC_ACTIVE]) {
        ...
        input.addEventListener('input', function() {
          ...
          var d = getNames();
          var filtered = term
            ? d.names.filter(function(n) {
                var email = getEmailForStudent(n, d.logs, d.grades, d.students);
                return n.toLowerCase().indexOf(term.toLowerCase()) !== -1 || email.toLowerCase().indexOf(term.toLowerCase()) !== -1;
              })
            : d.names;
          openDrop(filtered, term);
        });
        ...
        input.addEventListener('keydown', function(e) {
          ...
          if (e.key === 'ArrowDown') {
            ...
            e.preventDefault();
            if (AC_ACTIVE >= 0 && items[AC_ACTIVE]) {
              selectName(items[AC_ACTIVE].getAttribute('data-name'));
            } else if (AC_NAMES.length === 1) {
              selectName(AC_NAMES[0]);
            }
        ...
        /* init with pending student if any */
        if (PENDING_PROFILE_STUDENT) {
          input.value = PENDING_PROFILE_STUDENT;
          clear.style.display = 'flex';
      ...
      window.goToStudentProfile = function (name) {
        ...
        function openWhenReady() {
          ...
          if (!input.dataset.spInit) {
            PENDING_PROFILE_STUDENT = normalName;
            if (attempts++ < 10) setTimeout(openWhenReady, 120);
            return;
          }
          input.value = normalName;
          if (clear) clear.style.display = 'flex';
