...
define(['block_mwa_dashboard/dashboardstore', 'block_mwa_dashboard/engagementcalc'], function(Store, EngagementCalc) {
    ...
    (function () {
      ...
      function buildResourceCatalog(logs, activityLinks) {
        var map = {};
        (activityLinks || []).forEach(function (link) {
          if (!isResourceModule(link)) return;
      ...
      function buildStudentDetail(item, activityProgressHtml, resourceProgressHtml, detailId) {
        ...
        return '<div class="cl-detail-panel">'
          ...
          + '<div class="cl-detail-actions">'
            ...
            + '<button class="btn-ghost" type="button" onclick="window.MWAInterventions&&window.MWAInterventions.quickMessage(\'' + esc(item.name) + '\',\'' + esc(item.email || '') + '\',' + (item.userid || 0) + ',\'\');event.stopPropagation()">&#9993; ' + esc(tr('message')) + '</button>'
          + '</div>'
        + '</div>';
      }
      ...
      function renderClassListUnified() {
        ...
        var resources = buildResourceCatalog(logs, activityLinks);
