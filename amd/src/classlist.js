...
define(['block_mwa_dashboard/dashboardstore'], function(Store) {
    ...
    (function () {
      ...
      /* ── tr() reads Store.getStrings() directly (same pattern as dashboard) ── */
      function tr(key) {
      ...
      /* ── module state ── */
      var CL_VIEW = 'participation'; // 'participation' | 'activities'
      ...
      var SESSION_GAP = 30 * 60 * 1000;  // 30 min
      var MAX_SESSION = 60 * 60 * 1000;  // teto 60 min
      ...
      function calcSessionTimes(logs) {
        ...
        Object.keys(byStudent).forEach(function (n) {
          var times = byStudent[n].sort(function (a, b) { return a - b; });
          var total = 0;
          var sessionStart = times[0];
          var prev = times[0];
          for (var i = 1; i < times.length; i++) {
            var gap = times[i] - prev;
            if (gap > SESSION_GAP) {
              total += Math.min(prev - sessionStart + 2 * 60 * 1000, MAX_SESSION);
      ...
      /* ── email do aluno ── */
      function getEmailForStudent(name, logs) {
        var found = null;
      ...
      function calcParticipation(name, logs, grades) {
        // Cobertura de atividades (30%)
        var allActs = new Set();
        var myActs  = new Set();
        logs.forEach(function (r) {
          var ctx = norm(r.contextodoevento);
          if (!ctx || isCourseGeneral(ctx)) return;
          allActs.add(ctx);
          if (norm(r.nomecompleto) === name) myActs.add(ctx);
        });
        var coverScore = allActs.size > 0
          ? Math.min(100, Math.round((myActs.size / allActs.size) * 100))
          : 0;
        ...
        if (grades && grades.length) {
          grades.some(function (g) {
            // tenta casar pelo nome (parcial) ou email
            var gname = (norm(g['First name']) + ' ' + norm(g['Last name'])).trim();
            if (gname.toLowerCase() === name.toLowerCase()) {
              var k = Object.keys(g).find(function (x) {
                return x.toLowerCase().includes('course total') || x.toLowerCase().includes('total do curso');
              });
              if (k) {
                var n = parseFloat(String(g[k]).replace(',', '.'));
                if (!isNaN(n)) grade = n;
              }
              return true;
            }
        ...
        var hasDelivery = logs.some(function (r) {
          return norm(r.nomecompleto) === name && isSubmission(r);
        });
        var deliveryScore = hasDelivery ? 100 : 0;
    
        var hasGrades = grades && grades.length > 0;
        if (hasGrades && grade !== null) {
          return Math.round(gradeScore * 0.50 + coverScore * 0.30 + deliveryScore * 0.20);
        } else {
          // Sem nota: cap em 35% igual ao engagement.js — evita score alto por recência
          return Math.min(35, Math.round(coverScore * 0.70 + deliveryScore * 0.30));
        }
      }
      ...
      function buildActDots(name, grades) {
        ...
        // Usar o cache global de notas processado pelo grades.js (formato act_N)
        var cache = window.MWA_GRADES_CACHE || [];
        var actNames   = window.MWA_ACT_NAMES   || {};
        var actCmids   = window.MWA_ACT_CMIDS   || {};
        var actModules = window.MWA_ACT_MODULES  || {};
        var wwwroot    = (Store.getConfig().wwwroot || '');
    
        // Procura o aluno no cache processado
        var gradeObj = null;
        cache.some(function (g) {
          if (g.name.toLowerCase() === name.toLowerCase()) { gradeObj = g; return true; }
          return false;
        });
    
        // Fallback: tenta formato antigo (First name + Last name)
        if (!gradeObj) {
          var rawRow = null;
          grades.some(function (g) {
            if (g['__mwa_type__'] === 'activity_names') return false;
            var gname = (norm(g['First name']) + ' ' + norm(g['Last name'])).trim();
          ...
          // Formato antigo: usa actGrades do rawRow diretamente
          gradeObj = { actGrades: {} };
          Object.keys(rawRow).forEach(function (k) {
            ...
            if (m) {
              ...
              gradeObj.actGrades[m[1]] = isNaN(n) ? null : n;
            }
          });
        }
    
        var actGrades = gradeObj.actGrades || {};
        ...
        return '<div class="cl-act-dots">' + seqs.map(function (seq) {
          var val     = actGrades[String(seq)];
          var num     = (val === null || val === undefined) ? NaN : parseFloat(String(val).replace(',', '.'));
          var done    = !isNaN(num) && num > 0;
          var missing = val === null || val === undefined || val === '-' || val === '' || (!isNaN(num) && num === 0);
          var cls     = done ? 'done' : missing ? 'missing' : 'pending';
    
          // Tooltip: nome real da atividade
          var actName = actNames[Number(seq)] || ('Atividade ' + seq);
          var tooltip = esc(actName) + (done ? ' (' + num.toFixed(1) + ' pts)' : missing ? ' (—)' : '');
    
          // URL da atividade no Moodle
          var cmid   = actCmids[Number(seq)] || 0;
          var modname = actModules[Number(seq)] || '';
      ...
      function renderClassList() {
        ...
        entries.sort(function (a, b) {
          switch (sortMode) {
            ...
            case 'participation':
              var evData = window.MWAEngagement && window.MWAEngagement.getData ? window.MWAEngagement.getData() : [];
              var getScore = function(name) {
                var found = evData && evData.find(function(x) { return x.name === name; });
                return found ? found.score : calcParticipation(name, logs, grades);
              };
              return getScore(b.name) - getScore(a.name);
            case 'time':
              return (sessionTimes[b.name] || 0) - (sessionTimes[a.name] || 0);
            default: // interactions desc
              return b.count - a.count;
          }
        });
    
        // Update 4th column header according to mode
        var th4 = document.querySelector('#clTable thead tr th:nth-child(4)');
        if (th4) {
          th4.textContent = CL_VIEW === 'activities'
            ? tr('cl_th_deliveries')
            : tr('cl_th_participation');
        }
        ...
        var html = entries.map(function (e) {
          ...
          var part = (function(name) {
            var ev = window.MWAEngagement && window.MWAEngagement.getData ? window.MWAEngagement.getData() : [];
            var found = ev && ev.find(function(x) { return x.name === name; });
            return found ? found.score : calcParticipation(name, logs, grades);
          })(n);
          ...
          // Column 4: participation or activities
          var col4Html = CL_VIEW === 'activities'
            ? buildActDots(n, grades)
            : '<div class="cl-part-wrap">'
          ...
          return '<tr data-student="' + esc(n) + '">'
            + '<td style="width:27%;">'
              + '<div class="cl-student-cell">'
                + '<div class="cl-avatar" style="background:' + avatarBg + ';">' + esc(initials) + '</div>'
                + '<div style="min-width:0;flex:1;overflow:hidden;">'
                  + '<div class="cl-student-name" onclick="window.showPage&&window.showPage(\'studentprofile\')">' + esc(n) + '</div>'
                  + '<div class="cl-student-email">' + esc(email || tr('no_email')) + '</div>'
                + '</div>'
              + '</div>'
            + '</td>'
            + '<td style="width:14%;white-space:nowrap;">'
              + '<div style="font-size:.82rem;font-weight:600;color:' + ac + ';">' + fmtDate(last) + '</div>'
              + '<div class="cl-student-sub">' + cnt.toLocaleString('pt-BR') + ' ' + tr('interactions') + '</div>'
            + '</td>'
            + '<td style="width:8%;text-align:center;">'
              + '<span class="cl-days" style="color:' + ac + ';">' + ago + 'd</span>'
            + '</td>'
            + '<td style="width:22%;">' + col4Html + '</td>'
