...
define(['block_mwa_dashboard/dashboardstore', 'block_mwa_dashboard/engagementcalc'], function(Store, EngagementCalc) {
    ...
    (function () {
      ...
      function renderClassFilters(items, filtered) {
        ...
        Store.renderHtml(box, '<span class="cl-filter-label">' + esc(tr('ev_filter_label', 'Filtrar:')) + '</span>'
          + filters.map(function (f) {
            return '<button type="button" class="cl-filter-btn ' + (CL_FILTER === f[0] ? 'active' : '') + '" data-cl-filter="' + f[0] + '">' + esc(f[1]) + '</button>';
          }).join('')
          + '<span class="cl-filter-count">' + filtered.length + ' ' + esc(tr('students', 'estudantes')) + ' &middot; ' + esc(CL_FILTER === 'all' ? tr('ev_all', 'todos') : filters.filter(function (f) { return f[0] === CL_FILTER; })[0][1]) + '</span>');
      }
      ...
      function renderClassListUnified() {
        ...
        var cnt = document.getElementById('clCount');
        if (cnt) cnt.textContent = filtered.length + ' ' + tr('students', 'estudantes');
