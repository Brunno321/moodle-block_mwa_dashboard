...
define([], function() {
    ...
    function gradeInfo(name, email, grades) {
        ...
        if (row) {
            keys.forEach(function(key) {
                if (effectiveActivityValue(row, key, grades) !== null) {
                    launched++;
                }
            });
        }
    ...
    function calculateForStudent(name, email, logs, grades) {
        ...
        (logs || []).forEach(function(log) {
            ...
            if (!isCourseGeneral(activity)) {
                allActivities.add(lower(activity));
            }
            ...
            interactions++;
            if (activity && !isCourseGeneral(activity)) {
                studentActivities.add(lower(activity));
            }
            if (date) {
        ...
        var coverageScore = allActivities.size ? clamp((studentActivities.size / allActivities.size) * 100) : 0;
        ...
        var interactionTarget = Math.max(10, allActivities.size * 2);
