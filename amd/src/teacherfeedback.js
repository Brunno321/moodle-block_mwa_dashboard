...
define(['block_mwa_dashboard/dashboardstore', 'block_mwa_dashboard/engagementcalc'], function(Store, EngagementCalc) {
    ...
    function renderCharts(items) {
        ...
        var partial = items.filter(function(i) { return i.category === 'partial'; }).length;
        var advanced = Math.max(0, items.filter(function(i) { return i.advanced; }).length - partial);
        var inactive = items.filter(function(i) { return i.inactiveAfterContact || i.outcomeState === 'pending'; }).length;
        ...
        if (barsEl) {
            barsChart = new Chart(barsEl, {
                ...
                data: {
                    ...
                    datasets: [{data: [advanced, partial, inactive], backgroundColor: ['#5fb37d', '#6f9bdc', '#d96b63']}]
                },
    ...
    function renderWithData(items) {
        ...
        var html = renderFilters(items)
            ...
            + '<div class="card"><div class="card-head">&#9989; ' + esc(tr('tf_card_advanced', 'Quem avançou')) + '</div><div class="card-body">' + renderStudentList(advanced, 'advanced', true) + '</div></div>'
