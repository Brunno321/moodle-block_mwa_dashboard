...
define(['block_mwa_dashboard/dashboardstore', 'block_mwa_dashboard/engagementcalc'], function(Store, EngagementCalc) {
    ...
    function interventionOutcome(intervention, state) {
        ...
        if (type === 'activity_completion' && targets.length) {
            ...
            return delivered ?
                ...
                {state: 'pending', label: tr('int_result_pending', 'Pending'), advanced: false};
        }
    ...
    function trackedItemProgress(intervention, state) {
        ...
        return targets.map(function(target) {
            ...
            return {
                ...
                cmid: parseInt(target.cmid || 0, 10) || 0,
                mod: norm(target.mod || target.modtype || ''),
                url: norm(target.url || '')
            };
    ...
    function interventionLabel(count) {
        return count === 1 ? tr('tf_intervention_single', 'intervention') : tr('tf_interventions_short', 'interventions');
    }

    function returnedLabel(count) {
        return count === 1 ? tr('tf_returned_single', 'returned') : tr('tf_returned_plural', 'returned');
    }
    ...
    function statusText(status) {
        if (status === 'pending') {
            return tr('tf_filter_status_pending', 'Pending');
        }
        if (status === 'partial') {
            return tr('tf_filter_status_partial', 'Partial progress');
        }
        if (status === 'advanced') {
            return tr('tf_filter_status_advanced', 'Advanced');
        }
        return tr('tf_filter_all_statuses', 'All statuses');
    }
    ...
    function trackedItemsHtml(item) {
        ...
        var wwwroot = norm(cfg.wwwroot || '').replace(/\/$/, '');
        var doneCount = targets.filter(function(target) { return target.done; }).length;
        var pendingCount = targets.length - doneCount;
        var httpsPattern = new RegExp('^https?://', 'i');
        function targetUrl(target) {
            if (httpsPattern.test(target.url || '')) {
                return target.url;
            }
            if (wwwroot && /^\//.test(target.url || '')) {
                return wwwroot + target.url;
            }
            if (wwwroot && target.cmid && target.mod) {
                return wwwroot + '/mod/' + encodeURIComponent(target.mod) + '/view.php?id=' + encodeURIComponent(String(target.cmid));
            }
            return '';
        }
        return '<div class="tf-targets"><small>' + esc(tr('tf_detail_targets', 'Tracked items'))
            + ' <span class="tf-target-count">' + doneCount + ' ' + esc(tr('tf_legend_done', 'Done'))
            + ' &middot; ' + pendingCount + ' ' + esc(tr('tf_legend_pending', 'Pending')) + '</span></small>'
            + '<div class="tf-target-list">' + targets.map(function(target, index) {
                var status = target.done ? tr('tf_legend_done', 'Done') : tr('tf_legend_pending', 'Pending');
                var tooltip = target.name + ' — ' + status;
                var url = targetUrl(target);
                var content = String(index + 1);
                if (url) {
                    return '<a class="tf-target-dot ' + (target.done ? 'done' : 'pending') + '" href="' + esc(url)
                        + '" target="_blank" rel="noopener" title="' + esc(tooltip) + '" aria-label="' + esc(tooltip) + '">'
                        + content + '</a>';
                }
                return '<span class="tf-target-dot ' + (target.done ? 'done' : 'pending') + '" tabindex="0" title="'
                    + esc(tooltip) + '" aria-label="' + esc(tooltip) + '">' + content + '</span>';
            }).join('') + '</div></div>';
    }
    ...
    function renderStudentDetail(item, type) {
        ...
        return '<div class="tf-student-detail">'
            ...
            + '<div><small>' + esc(tr('tf_detail_interventions', 'Interventions received')) + '</small><strong>' + interventions.length + ' ' + esc(interventionLabel(interventions.length)) + '</strong></div>'
            + '<div><small>' + esc(tr('tf_detail_accessed_after', 'Accessed after')) + '</small>' + yesNoValue(returned) + '</div>'
            + '<div><small>' + esc(tr('tf_detail_delivered', 'Submitted something')) + '</small>' + yesNoValue(delivered) + '</div>'
            + '<div><small>' + esc(tr('tf_detail_grade', 'Improved grade')) + '</small>' + yesNoValue(gradeAfter) + '</div>'
            + '<div><small>' + esc(tr('tf_detail_reaction_time', 'Response time')) + '</small><strong>' + esc(delay) + '</strong></div>'
            + '</div>'
    ...
    function renderStudentList(items, type, alreadyGrouped) {
        if (!items.length) {
            return '<div class="tf-empty-small">' + esc(tr('tf_none', 'No students in this group.')) + '</div>';
        }
        ...
        function row(item) {
            ...
            if (type === 'risk' && item.pending) {
                chips.push(statusChip(tr('int_result_pending', 'Pending'), 'red'));
            } else {
                if (rows.some(function(entry) { return !!entry.returned; })) {
                    chips.push(statusChip(tr('tf_returned_short', 'returned'), 'blue'));
                }
                ...
                if (!rows.some(function(entry) { return !!entry.returned; })) {
                    chips.push(statusChip(tr('tf_still_risk_short', 'Still at risk'), 'red'));
                }
            }
        ...
        return '<div class="tf-student-list">' + visible.map(row).join('')
            + (hidden.length ? '<details class="tf-more-list"><summary>' + esc(tr('tf_show_all', 'Show all')) + ' (' + grouped.length + ')</summary>' + hidden.map(row).join('') + '</details>' : '')
            + '</div>';
    }
    ...
    function renderFilters(items) {
        ...
        return '<div class="tf-filter-panel">'
            + '<div class="tf-filter-field"><label>' + esc(tr('tf_filter_period', 'Period')) + '</label><select id="tfFilterPeriod">'
            + option('all', tr('tf_filter_all_periods', 'All periods'), filterState.period === 'all')
            + option('7', tr('tf_filter_7d', 'Last 7 days'), filterState.period === '7')
            + option('30', tr('tf_filter_30d', 'Last 30 days'), filterState.period === '30')
            + option('90', tr('tf_filter_90d', 'Last 90 days'), filterState.period === '90')
            + '</select></div>'
            + '<div class="tf-filter-field"><label>' + esc(tr('tf_filter_reason', 'Motivo')) + '</label><select id="tfFilterReason">'
            + option('all', tr('tf_filter_all_reasons', 'All reasons'), filterState.reason === 'all')
            ...
            + option('all', tr('tf_filter_all_statuses', 'All statuses'), filterState.status === 'all')
            + option('advanced', tr('tf_filter_status_advanced', 'Advanced'), filterState.status === 'advanced')
            + option('partial', tr('tf_filter_status_partial', 'Partial progress'), filterState.status === 'partial')
            + option('pending', tr('tf_filter_status_pending', 'Pending'), filterState.status === 'pending')
            + '</select></div>'
    ...
    function renderReasonRanking(items) {
        ...
        if (!rows.length) {
            return '<div class="tf-empty-small">' + esc(tr('tf_no_reason_data', 'Not enough reasons to rank.')) + '</div>';
        }
    ...
    function renderSummary(items) {
        ...
        var pendingText = noResponse ?
            esc(tr('tf_summary_pending_prefix', 'There are still')) + ' ' + noResponse + ' ' + esc(tr('tf_summary_pending_suffix', 'student(s) pending. This indicates that some contacts have not yet generated evidence of return, access, submission or grade.')) :
            esc(tr('tf_summary_no_pending_text', 'Currently, there are no students pending after contact. Keep monitoring to confirm progress in upcoming accesses.'));
        return '<div class="tf-summary-lead">'
            + '<strong>' + esc(tr('tf_summary_reading_title', 'Pedagogical reading')) + '</strong>'
            + '<span>' + esc(tr('tf_summary_reading_prefix', 'Recorded')) + ' <strong>' + total + '</strong> ' + esc(tr('tf_interventions_short', 'interventions')) + ' '
            + esc(tr('tf_summary_reading_middle', 'involving')) + ' <strong>' + unique + '</strong> ' + esc(tr('tf_summary_unique_students_suffix', 'unique student(s).')) + '</span>'
            + '</div>'
            ...
            + '</div>'
            + '<p><strong>' + esc(tr('tf_summary_impact_title', 'Observed impact')) + ':</strong> '
            + '<strong>' + returned + '</strong> ' + esc(tr('tf_summary_impact_returned', 'student(s) returned to Moodle,')) + ' '
            + '<strong>' + delivered + '</strong> ' + esc(tr('tf_summary_impact_delivered', 'made submissions and')) + ' '
            + '<strong>' + grade + '</strong> ' + esc(tr('tf_summary_impact_grade', 'had a new grade or performance record after the intervention.')) + '</p>'
            + '<p><strong>' + esc(tr('tf_summary_pending_title', 'Point of attention')) + ':</strong> ' + pendingText + '</p>'
            + '<p><strong>' + esc(tr('tf_summary_next_title', 'Suggested next action')) + ':</strong> '
            + esc(noResponse ? tr('tf_summary_next_pending', 'prioritize a short, contextualized message to pending students, mentioning the activity or content still to be completed.')
                : tr('tf_summary_next_monitor', 'monitor whether students who responded maintain access, submission and grade progress in the coming days.')) + '</p>'
            + '<p class="tf-summary-note">' + esc(tr('tf_summary_note', 'When a student has more than one intervention, the summary consolidates by unique student; the outcome charts count each intervention separately.')) + '</p>';
    }
    ...
    function renderCharts(items) {
        ...
        if (trendEl) {
            trendChart = new Chart(trendEl, {
                type: 'line',
                data: {
                    labels: [tr('rp_before', 'Before'), tr('rp_after', 'After')],
                    datasets: [{label: tr('tf_avg_engagement', 'Average engagement'), data: [beforeAvg, afterAvg], borderColor: '#5b9bd5', backgroundColor: 'rgba(91,155,213,.12)', fill: true, tension: .35}]
                },
        ...
        if (barsEl) {
            barsChart = new Chart(barsEl, {
                type: 'bar',
                data: {
                    labels: [tr('tf_advanced', 'Advanced'), tr('tf_partial', 'Partial progress'), tr('tf_pending_after_contact', 'Still pending')],
    ...
    function renderWithData(items) {
        ...
        if (!filtered.length) {
            Store.renderHtml(box, renderFilters(items)
                + '<div class="tf-empty-filter">' + esc(tr('tf_filter_empty', 'No interventions found with the selected filters.')) + '</div>');
            bindFilters();
            return;
        }
        ...
        var html = renderFilters(items)
            + '<div class="tf-filter-count">' + esc(tr('tf_filter_count_prefix', 'Showing')) + ' ' + filtered.length + ' ' + esc(tr('tf_filter_count_suffix', 'intervention(s) in the selection.')) + '</div>'
            + '<div class="tf-kpi-row">'
            + kpi(tr('tf_kpi_interventions_total', 'Interventions'), total, tr('tf_messages_sent', 'messages sent'), 'blue')
            + kpi(tr('tf_kpi_return', 'Return after contact'), pct(returned, totalStudents) + '%', returned + ' ' + tr('tf_unique_students', 'unique students'), 'blue')
            + kpi(tr('tf_kpi_engagement', 'Improved engagement'), pct(improvedEng, totalStudents) + '%', improvedEng + ' ' + tr('tf_unique_students', 'unique students'), 'green')
            + kpi(tr('tf_kpi_grade', 'Improved grade'), pct(improvedGrade, totalStudents) + '%', improvedGrade + ' ' + tr('tf_unique_students', 'unique students'), 'green')
            + kpi(tr('tf_kpi_risk', 'Still at risk'), pct(stillRisk, totalStudents) + '%', stillRisk + ' ' + tr('tf_pending_unique_students', 'pending students'), 'red')
            + '</div>'
            + '<div class="tf-chart-grid">'
            + '<div class="card"><div class="card-head">' + esc(tr('tf_chart_timeline', 'Timeline: before and after')) + '</div><div class="tf-chart"><canvas id="tfTrendChart"></canvas></div></div>'
            + '<div class="card"><div class="card-head">' + esc(tr('tf_chart_bars', 'Intervention outcomes')) + '</div><div class="tf-chart"><canvas id="tfBarsChart"></canvas></div></div>'
            + '</div>'
            + '<div class="tf-card-grid">'
            + '<div class="card"><div class="card-head">&#9989; ' + esc(tr('tf_card_advanced', 'Who progressed')) + '</div><div class="card-body">' + renderStudentList(advanced, 'advanced', true)
            + '<div class="tf-target-legend"><strong>' + esc(tr('tf_detail_targets', 'Tracked items')) + ':</strong><span><i class="tf-legend-dot red"></i>' + esc(tr('tf_legend_pending', 'Pending')) + '</span><span><i class="tf-legend-dot green"></i>' + esc(tr('tf_legend_done', 'Done')) + '</span></div>'
            + '</div></div>'
            + '<div class="card"><div class="card-head">&#128276; ' + esc(tr('tf_card_followup', 'Pending follow-up')) + '</div><div class="card-body">' + renderStudentList(needContact, 'risk', true) + '</div></div>'
            + '<div class="card"><div class="card-head">&#127942; ' + esc(tr('tf_card_effective', 'Most effective interventions')) + '</div><div class="card-body">' + renderReasonRanking(filtered) + '</div></div>'
            + '<div class="card"><div class="card-head">&#128221; ' + esc(tr('tf_card_summary', 'Teacher summary')) + '</div><div class="card-body"><div class="tf-summary">' + renderSummary(filtered) + '</div>'
    ...
    function renderEmpty() {
        ...
        Store.renderHtml(box, '<div class="gr-empty"><div class="gr-empty-icon">&#128200;</div><p>' + esc(tr('tf_no_data', 'Send interventions to see pedagogical effectiveness here.')) + '</p></div>');
    }
    ...
    function render() {
        ...
        try {
            Store.renderHtml(box, '<div class="empty"><div class="spinner"></div><p>' + esc(tr('tf_loading', 'Analyzing intervention effectiveness...')) + '</p></div>');
