...
define(['block_mwa_dashboard/dashboardstore'], function(Store) {
    ...
    function isAcademicAction(log) {
        ...
        return /submit|submitted|submission|attempt|tentativa|completed|completion|conclu|graded|grade|avaliad|post created|discussion created/.test(text);
    }
    ...
    function isResourceAccess(log) {
        ...
        if (['resource', 'page', 'url', 'book', 'folder', 'imscp'].indexOf(module) < 0) { return false; }
    ...
    function trackedActivityDone(target, current, afterLogs) {
        ...
        if (gradeRow && target.seq) {
            var currentCount = Number(gradeRow['act_current_' + target.seq] || 0);
            var currentGrade = parseFloat(String(gradeRow['act_' + target.seq] || '').replace(',', '.'));
            if (currentCount > 0 || (!isNaN(currentGrade) && currentGrade > 0)) { return true; }
        }
    ...
    function analyseRow(row, state, currentIndicators) {
        ...
        if (allTracked.length) {
            ...
            currentValue = currentIndicator ? number(currentIndicator.engagement) : null;
    ...
    function filtersHtml(rows) {
        ...
        var timestamps = rows.map(function(row) { return row.captured; }).filter(Boolean).sort();
        var rangeStart = filters.from || (timestamps.length ? new Date(timestamps[0] * 1000).toISOString().slice(0, 10) : '');
        var rangeEnd = filters.to || (timestamps.length ? new Date(timestamps[timestamps.length - 1] * 1000).toISOString().slice(0, 10) : '');
        var rangeLabel = rangeStart && rangeEnd ? new Date(rangeStart + 'T12:00:00').toLocaleDateString() +
            ' a ' + new Date(rangeEnd + 'T12:00:00').toLocaleDateString() : tr('tf_filter_all_period');
        return '<div class="fr-header-actions">' +
            '<button type="button" class="fr-btn fr-export" id="frExport">' + svg('download', 16) + ' Exportar relatório (PDF)</button>' +
            '<button type="button" class="fr-btn" id="frAdvanced" aria-expanded="' + (advancedOpen ? 'true' : 'false') + '">' + svg('filter', 16) + ' Filtros avançados</button>' +
            '<button type="button" class="fr-btn fr-period" id="frPeriod">' + svg('calendar', 16) + ' ' + esc(rangeLabel) + '</button></div>' +
            '<div class="fr-toolbar' + (advancedOpen ? ' open' : '') + '">' +
    ...
    function kpi(icon, label, value, sub, color, help) {
        return '<div class="fr-kpi" style="--fr-color:' + color + '"><div class="fr-kpi-head"><span class="fr-kpi-icon">' + svg(icon, 27) + '</span>' +
            '<span class="fr-kpi-label">' + esc(label) + (help ? '<span class="mwa-help-tip fr-kpi-help" tabindex="0" role="img" aria-label="' + esc(help) + '" title="' + esc(help) + '" data-tooltip="' + esc(help) + '">?</span>' : '') + '</span></div><small class="fr-kpi-sub">' + esc(sub) + '</small>' +
            '<strong class="fr-kpi-value">' + esc(value) + '</strong>' +
            '</div>';
    }
    ...
    function reasonChart(rows) {
        ...
        /* SVG dimensions */
        var W = 660, H = 220;
        var padL = 48, padR = 20, padT = 24, padB = 52;
        ...
        var bars = data.map(function(d, i) {
            ...
            /* X label with icon */
            var xLabel = '<text class="fr-axis-label fr-axis-label-x" x="' + cx + '" y="' + (H - 6) + '" text-anchor="middle" title="' + esc(d.label) + '">' +
                d.label.slice(0, 12) + (d.label.length > 12 ? '…' : '') + '</text>';
    ...
    function metricPairs(rows) {
        ...
        rows.forEach(function(row) {
            ...
            var currentEngagement = number(cur.engagement);
            if (initialEngagement !== null && currentEngagement !== null &&
    ...
    function lineChart(pairs) {
        ...
        var left = 48, right = 690, top = 18, bottom = 142;
        ...
        var paths = series.map(function(item) {
            ...
            item.values.forEach(function(value) {
                var day = new Date(value.captured * 1000).toISOString().slice(0, 10);
        ...
        var days = {};
        firstSeries.forEach(function(value) { days[new Date(value.captured * 1000).toISOString().slice(0, 10)] = true; });
        ...
        var firstDate = firstSeries.length ? formatDate(Math.min.apply(null, firstSeries.map(function(value) { return value.captured; }))) : '';
        return '<div class="fr-chart-title">' + tr('tf_linechart_title') + '</div><div class="fr-line-chart"><svg viewBox="0 0 710 170" role="img" aria-label="' + esc(tr('tf_linechart_aria')) + '"><g class="grid">' + grid + '</g>' +
            '<line class="fr-intervention-line" x1="' + left + '" y1="' + top + '" x2="' + left + '" y2="' + bottom + '"/><text class="fr-intervention-label" x="' + (left + 7) + '" y="28">' + tr('tf_linechart_marker') + '</text><text class="fr-intervention-label" x="' + (left + 7) + '" y="38">(' + esc(firstDate) + ')</text><g class="lines">' + paths + '</g>' + xlabels +
    ...
    function continuityHtml(groups) {
        var c7 = continuity(groups, 0, 7), sustained = sustainedAfterSeven(groups);
        var total = groups.length, responded = groups.filter(function(group) { return group.responded; }).length;
        var now = Date.now() / 1000;
        ...
        var respondedGroups = groups.filter(function(group) { return group.responded; });
        var steps = [
            [total, tr('tf_funnel_received'), '100%', groups.map(function(group) { return group.name; })],
            [responded, tr('tf_funnel_responded'), pctText(responded, total), respondedGroups.map(function(group) { return group.name; })],
            [c7.count, tr('tf_funnel_active7'), pctText(c7.count, total), c7.groups.map(function(group) { return group.name; })],
            [sustained.count, tr('tf_funnel_sustained'), pctText(sustained.count, total), sustained.groups.map(function(group) { return group.name; })]
        ];
        return '<div class="fr-funnel">' + steps.map(function(step, index) {
            var description = step[1] + ': ' + step[2];
            return '<div class="fr-funnel-step" tabindex="0" role="button" data-fr-funnel="' +
                esc(JSON.stringify({label: step[1], names: step[3]})) + '" title="' + esc(description) + '" aria-label="' + esc(description) + '">' +
                '<i class="fr-funnel-shape fr-funnel-' + index + '">' + step[0] + '</i>' +
                '<span class="fr-funnel-label fr-funnel-label-' + index + '">' + step[1] + '</span>' +
                '<em class="fr-funnel-tooltip" aria-hidden="true">' + step[2] + '</em></div>';
        }).join('') + '</div><div class="fr-continuity-side"><h3>' + tr('tf_cont_side_title') + '</h3>' +
    ...
    function buildAiPrompt(rows) {
        ...
        var lines = groups.slice(0, 100).map(function(group) {
            ...
            var afterEngagement = row.currentIndicator ? number(row.currentIndicator.engagement) : null;
    ...
    function renderReport(allRows) {
        ...
        var html = filtersHtml(allRows) + '<div class="fr-kpis">' +
        ...
        html += '<div class="fr-grid fr-grid-top">' +
            card(tr('tf_reason_chart_title'), reasonChart(rows), 'overview engagement', 'fr-span-8 fr-result-card') +
            card('Interação do estudante após a intervenção', interactionHtml(rows, groups), 'overview interaction', 'fr-span-6 fr-interaction-card') + '</div>';
        html += '<div class="fr-grid fr-grid-middle">' +
            card(tr('tf_progress_card_title'), '<div class="fr-deltas">' +
                ...
                '</div>' + lineChart(pairs) + info(tr('tf_progress_info')), 'overview learning', 'fr-span-8') +
            card(tr('tf_card_synthesis'), donutHtml(groups), 'overview', 'fr-span-6 fr-summary-card') + '</div>';
        html += aiReportHtml();
        html += '<div class="fr-grid fr-grid-bottom">' +
            card(tr('tf_card_continuity'), continuityHtml(groups), 'overview continuity', 'fr-span-8') +
            card(tr('tf_card_mediation_time'), mediationHtml(rows), 'overview mediation', 'fr-span-6 fr-mediation-card') +
        ...
        html += '<footer class="fr-footer"><span>ⓘ Dados calculados com snapshots históricos e registros atuais do Moodle.</span><b>MWA Analytics · Learning Analytics</b><span>Atualizado em ' + formatDateTime(Math.floor(Date.now() / 1000)) + '</span></footer>';
        Store.renderHtml(box, html);
        applyTab();
        bind(allRows);
    }
    ...
    function bind(rows) {
        ...
        if (exportButton) { exportButton.addEventListener('click', function() { window.print(); }); }
        ...
        var advanced = document.getElementById('frAdvanced');
        var period = document.getElementById('frPeriod');
        function toggleAdvanced() { advancedOpen = !advancedOpen; renderReport(rows); }
        if (advanced) { advanced.addEventListener('click', toggleAdvanced); }
        if (period) { period.addEventListener('click', toggleAdvanced); }
